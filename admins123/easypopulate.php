...
//
// v1.2.5.8.csv 2013/06/1 juns
// v1.2.5.9.csv 2013/07/31 juns
//
// *******************************
// *******************************
...
/*
 * put below @set_time_limit(1200); @ini_set('max_input_time','1200'); //
 */
/**
...
/*
 * Add your custom fields to this array these will automatically add the fields in the necessary sql statement and other arrays functions both import and export NOTE: Currently just works on TABLE_PRODUCTS
 */
$custom_fields == FALSE;
...
// TODO: add new csv format allow all images in csv union
$down_image_file_total = 10000;
$down_image_count = 0;
function getDownImageFile($dataCsv)
{
	// return new or last
	global $down_image_csv, $tempdir;
	$down_dir = dirname(__FILE__) . '/down';
	if (!is_dir($down_dir)) @mkdir($down_dir);
	if (!isset($down_image_csv) || empty($down_image_csv))
	{
		if (strpos($dataCsv, '.') !== false) $temp = substr($dataCsv, 0, strrpos($dataCsv, '.'));
		$down_image_csv = $temp . '-list.csv';
		$down_image_csv = str_replace($tempdir, 'down/', $down_image_csv);
		if (file_exists($down_image_csv)) 		// replay
		unlink($down_image_csv);
	}
	return $down_image_csv;
}
...
if (is_array($attribute_options_select) && (count($attribute_options_select) > 0))
{
	...
	foreach($attribute_options_select as $value)
	{
		...
		{
			$attribute_options_array[] = array(
				'products_options_id' => $attribute_options['products_options_id'] 
			);
		}
	}
}
...
// TODO: all exists attributes[property]
// option[lang_id][option_id]
$property_option_type = array();
$property_option = array();
$property_option_value = array();

$property_option_type_query = ep_query('SELECT * FROM ' . TABLE_PRODUCTS_OPTIONS_TYPES);
while($pr = mysql_fetch_array($property_option_type_query))
{
	$property_option_type[$pr['products_options_types_id']] = $pr['products_options_types_id'];
}
$property_option_query = ep_query('SELECT * FROM ' . TABLE_PRODUCTS_OPTIONS . ' ORDER BY `products_options_id` ASC');
while($pr = mysql_fetch_array($property_option_query))
{
	$property_option[$pr['language_id']][$pr['products_options_id']] = $pr['products_options_name'];
}
$property_option_value_query = ep_query('SELECT * FROM ' . TABLE_PRODUCTS_OPTIONS_VALUES . ' ORDER BY `products_options_values_id` ASC');
while($pr = mysql_fetch_array($property_option_value_query))
{
	$property_option_value[$pr['language_id']][$pr['products_options_values_id']] = $pr['products_options_values_name'];
}
	...
	switch($ep_dltype)
	{
		case 'full':
			...
			foreach($langcode as $key => $lang)
			{
				...
				if ($ep_supported_mods['psd'] == true) $filelayout[] = 'v_products_short_desc_' . $l_id;
			...
			p.metatags_title_tagline_status as v_metatags_title_tagline_status" . $custom_filelayout_sql . " FROM
			" . TABLE_PRODUCTS . " as p,
		...
		case 'priceqty':
			...
			/*
			 * $filelayout[] = 'v_customer_price_1'; $filelayout[] = 'v_customer_group_id_1'; $filelayout[] = 'v_customer_price_2'; $filelayout[] = 'v_customer_group_id_2'; $filelayout[] = 'v_customer_price_3'; $filelayout[] = 'v_customer_group_id_3'; $filelayout[] = 'v_customer_price_4'; $filelayout[] = 'v_customer_group_id_4'; $filelayout[] = 'v_last_modified'; $filelayout[] = 'v_status'; //
			 */
		...
		case 'froogle':
			...
			p.products_quantity as v_products_quantity,
			p.manufacturers_id as v_manufacturers_id,
			subc.categories_id as v_categories_id" . $custom_filelayout_sql . " FROM
			" . TABLE_PRODUCTS . " as p,
			" . TABLE_CATEGORIES . " as subc,
			" . TABLE_PRODUCTS_TO_CATEGORIES . " as ptoc
			WHERE
...
if ($ep_dlmethod == 'stream' or $ep_dlmethod == 'tempfile')
{
	...
	while($row = mysql_fetch_array($result, MYSQL_ASSOC))
	{
		...
		/*
		 * if ( $row['v_status'] == '1' ){ $row['v_status'] = $active; } else { $row['v_status'] = $inactive; }
		 */
...
if (isset($_POST['localfile']) || isset($_FILES['usrfl']))
{
	...
	//TODO: detect shop id
	$shop_id = 1;
	$shop_basename = basename($file_location);
	$shop_basename = trim(substr($shop_basename, 0, strrpos($shop_basename, '.')));
	$shop_basename = explode('-', $shop_basename);
	$shop_id = (int)end($shop_basename);
	if ($shop_id <= 0)
		$shop_id = 1;
	// *******************************
	// *******************************
	...
	else if ($filelayout = array_flip(fgetcsv($handle, 0, $csv_deliminator, $csv_enclosure)))
	{
		while($items = fgetcsv($handle, 0, $csv_deliminator, $csv_enclosure))
		{
			...
			while($row = mysql_fetch_array($result))
			{
				...
				/*
				 * Get current products descriptions and categories for this model from database $row at present consists of current product data for above fields only (in $sql)
				 */
			...
			foreach($langcode as $lang)
			{
				...
				// TODO: new property set values
				if (isset($filelayout['v_property_' . $l_id]))
				{
					$v_property[$l_id] = $items[$filelayout['v_property_' . $l_id]];
				}
				// TODO: end property values set
			}
			
			// TODO: add new csv formart
			$v_all_images = $items[$filelayout['v_all_images']];
			$items['shop_id'] = $shop_id;
			$items['v_shop_id'] = $shop_id;
			...
			{ // does category 1 column exist in our file..
			...
			if (isset($v_categories_name_1))
			{
				...
				for($categorylevel = $max_categories + 1; $categorylevel > 0; $categorylevel--)
				{
					...
					if ($thiscategoryname != '')
					{
						...
						$result = ep_query($sql);
						if ($row = mysql_fetch_array($result))
						{ // langer - null result here where len of $v_categories_name[] exceeds maximum in database
							$thiscategoryid = $row['catID'];
						}
						else
						{
							// to add, we need to put stuff in categories and categories_description
							$sql = "INSERT INTO " . TABLE_CATEGORIES . " ( parent_id, sort_order, date_added, last_modified ,shop_id)
								VALUES ( $theparent_id, 0, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, $shop_id )";
							$result = ep_query($sql);
							
							$thiscategoryid = mysql_insert_id();
							
							$sql = "INSERT INTO " . TABLE_CATEGORIES_DESCRIPTION . "( categories_id, language_id, categories_name,shop_id )
								VALUES ( $thiscategoryid, '$epdlanguage_id', '" . zen_db_input($thiscategoryname) . "', $shop_id )";
							$result = ep_query($sql);
						}
			...
			if ($v_products_model != "")
			{
				...
				$result = ep_query("SELECT `products_id` FROM " . TABLE_PRODUCTS . " WHERE 
						( shop_id=".$shop_id." and products_model = '" . zen_db_input($v_products_model) . "') LIMIT 1 ");
				...
				if ($row = mysql_fetch_array($result))
				{
					...
					if (!$v_date_added && $row['v_date_added'])
					{
						$v_date_added = $row['v_date_added'];
					}
					...
					$query = "UPDATE " . TABLE_PRODUCTS . " SET
						...
						metatags_title_tagline_status	=	'" . zen_db_input($v_metatags_title_tagline_status) . "'" . $custom_query . "WHERE ( `products_id` = '" . $v_products_id . "' ) ";
				...
				// TODO: this add new property
				/**
				 * property format: Color-[option_id]:Value1|Value2;Size-[option_type_id]:Value1|Value2
				 * eg: Color-2:Black|Blue|Gray;Size-0:12CM|24CM|12CM=24CM=32CM
				 */
				if (isset($v_property) && count($v_property) > 0)
				{
					$languages = zen_get_languages();
					...
					for($i = 0, $n = sizeof($languages); $i < $n; $i++)
					{
						...
						if (!isset($v_property[$lid]) || empty($v_property[$lid])) continue;
						
						$v_property_curr_var = $v_property[$lid];
						// split value
						$prop_mult = explode(";", $v_property_curr_var);
						foreach($prop_mult as $p_var)
						{
							$prop_var = explode(':', $p_var);
							
							$prop_tmp = explode("-", $prop_var[0]);
							if (count($prop_tmp) < 2)
							{
								$prop_tmp_t = $prop_tmp;
								$prop_tmp = array();
								$prop_tmp = array_shift($prop_tmp_t);
								$prop_tmp[1] = 0; // option type eg:0,1,2,3
							}
							
							$prop_name = trim($prop_tmp[0]);
							$prop_type_id = $prop_tmp[1];
							
							$prop_values = explode('|', $prop_var[1]);
							foreach ($prop_values as $p_key => $p_value)
							{
								$p_value = trim($p_value);
								if (empty($p_value))
									unset($prop_values[$p_key]);
							}
							$prop_values = array_unique($prop_values);
							// TODO: add property price parse
							// parse value price
							foreach($prop_values as $k => $p_v)
							{
								$p_v_t = array();
								if (strpos($p_v, '{') != false)
								{ // has price
									$p_v = str_replace('}', '', $p_v);
									$p_v_multi = explode('{', $p_v);
									if (count($p_v_multi) < 2) $p_v_multi[1] = 0;
									
									$p_v_t = array(
										'name' => trim($p_v_multi[0]),
										'price' => $p_v_multi[1] 
									);
								}
								else
							...
							if (!array_key_exists($prop_type_id, $property_option_type))
							{ // not option type id
							                                                             // TODO: report not type
								continue;
							}
							$prop_option_id = 0;
							if (!in_array($prop_name, (array)$property_option[$lid]))
							{
								// create option
								// like Color or Size...
								// set $prop_option_id
								$tmp_arr = array_flip((array)$property_option[$lid]);
								$prop_option_id = (int)end($tmp_arr);
								++$prop_option_id;
								
								$prop_sql1 = "INSERT INTO `" . TABLE_PRODUCTS_OPTIONS . "` (`products_options_id`, `language_id`, `products_options_name`, 
									`products_options_sort_order`, `products_options_type`, `products_options_length`, `products_options_comment`, 
									`products_options_size`, `products_options_images_per_row`, `products_options_images_style`, 
									`products_options_rows`) VALUES ('%s', '%s', '%s', '0', '%s', '32', NULL, '32', '0', '0', '0');";
								ep_query(sprintf($prop_sql1, (int)$prop_option_id, (int)$lid, $db->prepare_input($prop_name), (int)$prop_type_id)); // TODO: add option type
								
								$property_option[$lid][$prop_option_id] = $prop_name;
							}
							else
							{
								foreach($property_option[$lid] as $prp_id => $v)
								{
									if ($v == $prop_name)
									{
										$prop_option_id = $prp_id;
										break;
									}
								}
							}
							// option value
							$prop_value_ids = array();
							foreach($prop_values as $prop_val)
							{
								if (!in_array($prop_val['name'], (array)$property_option_value[$lid]))
								{
									// create option value to option
									// TODO: option value not in option[products_options_values_to_products_options]
									// set $prop_value_ids
									$prop_id = 0; // insert result id
									$tmp_arr = array_flip((array)$property_option_value[$lid]);
									$prop_id = (int)end($tmp_arr);
									++$prop_id;
									
									$prop_sql2 = "INSERT INTO `" . TABLE_PRODUCTS_OPTIONS_VALUES . "` (`products_options_values_id`, `language_id`, 
										`products_options_values_name`, `products_options_values_sort_order`) VALUES ('%s', '%s', '%s', '0');";
									ep_query(sprintf($prop_sql2, (int)$prop_id, (int)$lid, $db->prepare_input($prop_val['name'])));
									
									// is new option values, need insert
									// just insert default language
									if ($lid == $epdlanguage_id)
									{
										$prop_sql3 = "INSERT INTO `" . TABLE_PRODUCTS_OPTIONS_VALUES_TO_PRODUCTS_OPTIONS . "`( `products_options_id`, 
											`products_options_values_id`) VALUES ('%s','%s');";
										ep_query(sprintf($prop_sql3, (int)$prop_option_id, (int)$prop_id));
									}
									
									$property_option_value[$lid][$prop_id] = $prop_val['name'];
									$prop_value_ids[$prop_id] = array(
										'id' => $prop_id,
										'price' => $prop_val['price'] 
									);
								}
								else
								{
									foreach($property_option_value[$lid] as $prop_id => $v)
									{
										if ($v == $prop_val['name'])
										{
											$property_option_value[$lid][$prop_id] = $v;
											$prop_value_ids[$prop_id] = array(
												'id' => $prop_id,
												'price' => $prop_val['price'] 
											);
							...
							{
								$property_product_query = 'INSERT INTO `' . TABLE_PRODUCTS_ATTRIBUTES . '` (`products_attributes_id`, `products_id`, `options_id`, 
									`options_values_id`, `options_values_price`, `price_prefix`, `products_options_sort_order`, `product_attribute_is_free`, 
									`products_attributes_weight`, `products_attributes_weight_prefix`, `attributes_display_only`, `attributes_default`, 
									`attributes_discounted`, `attributes_image`, `attributes_price_base_included`, `attributes_price_onetime`, 
									`attributes_price_factor`, `attributes_price_factor_offset`, `attributes_price_factor_onetime`, 
									...
									`attributes_required`) VALUES ';
								$prop_values_array = array();
								$prop_sql_format = "(NULL, '%s', '%s', '%s', '%s', '+', '0', '0', '0', '', '0', '0', '1', NULL, '1', '0.0000', '0.0000', '0.0000', '0.0000', '0.0000', NULL, NULL, '0.0000', '0', '0.0000', '0', '0')";
								foreach($prop_value_ids as $id => $v)
								{
									$prop_values_array[] = sprintf($prop_sql_format, $v_products_id, $prop_option_id, $id, $v['price']);
								}
								// delete from [products_attributes] products_id and options_id
								ep_query('DELETE FROM ' . TABLE_PRODUCTS_ATTRIBUTES . ' WHERE `products_id`=' . $v_products_id . ' AND `options_id`=' . $prop_option_id);
								
								$result_product = ep_query($property_product_query . implode(',', $prop_values_array)); // insert product option
				...
				// TODO: end new property
				
				// *************************
				// TODO: add new csv format allow all images in csv union
				// *************************
				
				if (isset($v_all_images) && !empty($v_all_images) && strpos($v_all_images, 'a:') === 0)
				{
					$query_product_iamge = $db->Execute('SELECT COUNT(*) AS total FROM products_images WHERE products_id='.$v_products_id);
					if ($query_product_iamge->fields['total'] > 0)
					{
						$db->Execute("UPDATE products_images SET images='".$db->prepare_input($v_all_images)."' WHERE products_id=".$v_products_id);
					}
					else
					{
						$db->Execute("INSERT products_images(`products_id`,`images`)VALUES({$v_products_id}, '{$db->prepare_input($v_all_images)}');");
					}
					$v_all_images = unserialize($v_all_images);
					$down_image_file = getDownImageFile($file_location);
					foreach($v_all_images as $img)
					{
						file_put_contents($down_image_file, implode(',', $img) . "\n", FILE_APPEND);
					}
				}
				
				// *************************
				// TODO: end add new csv format allow all images in csv union
				// *************************
				...
				/**
				 * Specials
				 * if a null value in specials price, do not add or update.
				 * If price = 0, let's delete it
				 */
							...
							<table width="70%" border="0" cellpadding="8" valign="top">
								<tr>
									<td width="100%">
										...
										<form ENCTYPE="multipart/form-data" ACTION="easypopulate.php"
											METHOD="POST">
											<div align="left">
												<b>上传批量文件</b><br /> <input TYPE="hidden"
													name="MAX_FILE_SIZE" value="100000000"> <input name="usrfl"
													type="file" size="50"> <input type="submit"
													name="buttoninsert" value="导入数据库"> <br />
											</div>
										</form> <br>
										<form ENCTYPE="multipart/form-data" ACTION="easypopulate.php"
											METHOD="POST">
											<div align="left">
												<b>从临时目录导入 (<?php echo $tempdir; ?>)</b><br /> <input
													TYPE="text" name="localfile" size="50"> <input
													type="submit" name="buttoninsert" value="导入数据库"> <br />
											</div>
										</form>
										</p> <b>下载批量和Froogle文件</b> <br />
									<br /> <!-- Download file links -  Add your custom fields here -->
										<a href="easypopulate.php?download=stream&dltype=full">下载<b>完整的</b>.csv文件
									</a>
...
<?php } ?>
										<br /> <a
										href="easypopulate.php?download=stream&dltype=priceqty">下载 <b>型号/价格/数量</b>
											的.csv文件
									</a><br /> <a
										href="easypopulate.php?download=stream&dltype=modqty">下载 <b>型号/价格/数量/最后更新/状态</b>
											的.csv文件
									</a><br /> <a
										href="easypopulate.php?download=stream&dltype=priceqty">下载 <b>型号/价格/数量</b>
											的.csv文件
									</a><br /> <a
										href="easypopulate.php?download=stream&dltype=froogle">下载 <b>Froogle</b>
											的Tab分割的.txt文件
									</a><br /> <a
										href="easypopulate.php?download=stream&dltype=attrib">下载 <b>型号/属性</b>
											的.csv文件
									</a> <br />
									<br /> <b>在临时目录中创建批量和Froogle文件: <a
											href="../<?php echo $tempdir; ?>"><?php echo $tempdir; ?></a></b>
										<br />
									<br /> <a href="easypopulate.php?download=tempfile&dltype=full">在临时目录中创建
											<b>完整的</b> .txt文件
									</a>
<?php if ($products_with_attributes == true) { ?>
										<span class="fieldRequired"> (包含属性)</span>
...
<?php } ?>
										<br /> <a
										href="easypopulate.php?download=tempfile&dltype=priceqty">在临时目录中创建
											<b>型号/价格/数量</b> 的.csv文件
									</a><br /> <a
										href="easypopulate.php?download=tempfile&dltype=modqty">在临时目录中创建
											<b>型号/价格/数量/最后更新/状态</b> 的.csv文件
									</a><br /> <a
										href="easypopulate.php?download=tempfile&dltype=category">在临时目录中创建
											<b>型号/分类</b> 的.csv文件
									</a><br /> <a
										href="easypopulate.php?download=tempfile&dltype=froogle">在临时目录中创建
											<b>Froogle</b> 的Tab分割的.txt文件
									</a><br /> <a
										href="easypopulate.php?download=tempfile&dltype=attrib">在临时目录中创建
											<b>型号/属性</b> 的.csv文件
									</a><br />
									</td>
								</tr>
