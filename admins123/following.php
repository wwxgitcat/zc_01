...
if (isset($_GET['down']) && !empty($_GET['down']) && in_array($_GET['down'], $down_types))
{
	...
	$filed_format = 'customers_follow,customers_id,customer_status,orders_id,identify,date_add,active,sended,sended_date,referer,user_agent,keyword,jcoupon_status,jcoupon_get,jcoupon_start_time,jcoupon_end_time'."\n";
	switch ($_GET['down'])
	{
		case 'csv':
			...
			$result = $db->Execute('SELECT * FROM '.DB_PREFIX.'customers_follow ORDER BY customers_follow ASC');
			while (!$result->EOF)
			{
				$status = '账号没跟踪到';
				if (!$result->fields['active'] && !$result->fields['sended'])
					...
					$status = '正常账号';
				$field = array(
					$result->fields['customers_follow'],
					$result->fields['customers_id'],
					$status,
					$result->fields['orders_id'],
					$result->fields['identify'],
					$result->fields['date_add'],
					$result->fields['active'],
					$result->fields['sended'],
					$result->fields['sended_date'],
					$result->fields['referer'],
					$result->fields['user_agent'],
					$result->fields['keyword'],
					$result->fields['jcoupon_status'],
					$result->fields['jcoupon_get'],
					$result->fields['jcoupon_start_time'],
					$result->fields['jcoupon_end_time']
				);
				foreach ($field as &$ff)
				{
					$ff = str_replace(',', ' ', $ff);
