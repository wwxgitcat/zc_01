...
if (basename($_SERVER['SCRIPT_FILENAME']) != FILENAME_ALERT_PAGE . '.php')
{
	...
	if (!(basename($PHP_SELF) == FILENAME_LOGIN . ".php"))
	{
		...
		if (!isset($_SESSION['admin_id']))
		{
			...
			{
				if (!defined('CRON_NOT_LOGIN_YES') || CRON_NOT_LOGIN_YES != true) zen_redirect(zen_href_link(FILENAME_LOGIN, 'camefrom=' . basename($PHP_SELF) . '&' . zen_get_all_get_params(), 'SSL'));
			}
		}
		...
		)) && !zen_is_superuser())
		{
			...
			{
				if (!defined('CRON_NOT_LOGIN_YES') || CRON_NOT_LOGIN_YES != true) zen_redirect(zen_href_link(FILENAME_DENIED, '', 'SSL'));
			}
		}
	}
