	...
	<tr>
		<td><table border="0" width="100%" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td align="right"><table border="0" width="100%" cellspacing="0"
							cellpadding="0">
							<tr>
...
<?php if ($action == '') { ?>
                <td class="dataTableHeadingContent" width="20"
									align="right"><?php echo TABLE_HEADING_ID; ?></td>
...
if (isset($_GET['search']))
{
	...
	$categories = $db->Execute("select c.categories_id, cd.categories_name, cd.categories_description, c.categories_image,
                                         ...
                                         c.categories_status, c.is_top
                                  from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd
                                  where c.categories_id = cd.categories_id
                                  and cd.language_id = '" . (int)$_SESSION['languages_id'] . "'
                                  and cd.categories_name like '%" . zen_db_input($search) . "%'" . $order_by);
}
else
{
	$categories = $db->Execute("select c.categories_id, cd.categories_name, cd.categories_description, c.categories_image,
                                  ...
                                  and cd.language_id = '" . (int)$_SESSION['languages_id'] . "'" . $order_by);
}
...
if (isset($_GET['search']) && !empty($_GET['search']) && $action != 'edit_category')
{
	...
	$products_query_raw = ("select p.products_type, p.products_id, pd.products_name, p.products_quantity,
                                ...
                                or p.products_model like '%" . zen_db_input($_GET['search']) . "%')" . $order_by);
}
else
{
	$products_query_raw = ("select p.products_type, p.products_id, pd.products_name, p.products_quantity,
                                ...
                                and pd.language_id = '" . (int)$_SESSION['languages_id'] . "'
                                and p.products_id = p2c.products_id
                                and p2c.categories_id = '" . (int)$current_category_id . "'" . $order_by);
}
// Split Page
...
<form name="newproduct"
													action="<?php echo zen_href_link(FILENAME_CATEGORIES, '', 'NONSSL'); ?>"
													method="get"><?php echo (empty($_GET['search']) ? zen_image_submit('button_new_product.gif', IMAGE_NEW_PRODUCT) : ''); ?>
<?php
		...
		if ($restrict_types->RecordCount() > 0)
		{
			...
			{
				$product_restrict_types_array[] = array(
					'id' => $restrict_types->fields['product_type_id'],
...
echo '&nbsp;&nbsp;' . zen_draw_pull_down_menu('product_type', $product_restrict_types_array);
		...
		?>
           <input type="hidden" name="cPath"
														value="<?php echo $cPath; ?>"> <input type="hidden"
														name="action" value="new_product">
												</form>
<?php
	}
	else
