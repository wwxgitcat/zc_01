...
class PHPlot
{
	...
	function SetInputFile($which_input_file)
	{
		...
		switch($input_type)
		{ // After SetFileFormat is in lower case
			case "1":
	...
	function SetDefaultColors()
	{
		...
		$this->SetBackgroundColor(array(
			...
			222,
			222 
		)); // can use rgb values or "name" values
		$this->SetLabelColor('black');
		$this->SetTextColor('black');
		$this->SetGridColor('black');
		$this->SetLightGridColor(array(
			175,
			175,
			175 
		));
		$this->SetTickColor('black');
		$this->SetTitleColor(array(
			0,
			0,
			0 
		)); // Can be array or name
		$this->data_color = array(
			'blue',
			'green',
			'yellow',
			'red',
	...
	function DrawBackground()
	{
		...
		if ($this->background_done == 0)
		{ // Don't draw it twice if drawing two plots on one image
	...
	function FindDataLimits()
	{
		...
		while(list($dat_key, $dat) = each($this->data_values))
		{ // for each X barchart setting
	...
	function SetMargins()
	{
		...
		{
			$title_size = array(
				ImageFontWidth($this->title_font) * strlen($this->title_txt),
	...
	function SetPlotAreaWorld($xmin, $ymin, $xmax, $ymax)
	{
		if (($xmin == "") && ($xmax == ""))
		{
			...
			{ // labels for text-data is done at data drawing time for speed.
	...
	function SetRGBArray($which_color_array)
	{
		...
		elseif ($which_color_array == 2)
		{ // Use the small predefined color array
	...
	function SetRgbColor($color_asked)
	{
		...
		{ // is asking for a color by string
			if (substr($color_asked, 0, 1) == "#")
			{ // asking in #FFFFFF format.
				$ret_val = array(
					hexdec(substr($color_asked, 1, 2)),
	...
	function DrawXAxis()
	{
		...
		// X Ticks and Labels
		if ($this->data_type != 'text-data')
		{ // labels for text-data done at data drawing time for speed.
	...
	{
		if ($this->skip_top_tick != 1)
		{ // If tick increment doesn't hit the top
		                                 // Left Top
	...
	function DrawDataLabel($lab, $x_world, $y_world)
	{
		...
		{
			$lab_size = array(
				$this->small_font_width * StrLen($lab),
	...
	function DrawXDataLabel($xlab, $xpos)
	{
		...
		{
			$xlab_size = array(
				ImageFontWidth($this->axis_font) * StrLen($xlab),
				$this->small_font_height * 3 
			);
			if ($this->x_datalabel_angle == 90)
			{
	...
	function DrawLines()
	{
		...
		while(list($j, $row) = each($this->data_values))
		{
			...
			while(list($k, $v) = each($row))
			{
				...
				if (($this->draw_x_data_labels == 1) && ($k == 1))
				{ // See "labels_note1 above.
				...
				else
				{
					...
					{ // Allow for missing Y data
						...
						{ // ajo
							...
							$this->DrawText($this->small_font, $this->x_label_angle, $x1 + $this->record_bar_width / 2, $y1, $this->ndx_label_color, $this->x_label_ttffont_size, $v, 'center', 'top');
						}
					}
	...
	function DrawGraph()
	{
		...
		else
		{
			...
			if (!$this->plot_max_y)
			{ // If not set by user call SetPlotAreaWorld,
