...
function zen_get_all_get_params($exclude_array = '')
{
	...
	$exclude_array = array_merge($exclude_array, array(
		'main_page' 
	));
	$get_url = '';
...
function zen_get_category_tree($parent_id = '0', $spacing = '', $exclude = '', $category_tree_array = '', $include_itself = false, $category_has_products = false, $limit = false)
{
	...
	if ((sizeof($category_tree_array) < 1) && ($exclude != '0')) $category_tree_array[] = array(
		'id' => '0',
		'text' => TEXT_TOP 
	);
	...
	while(!$categories->EOF)
	{
		...
		if ($exclude != $categories->fields['categories_id']) $category_tree_array[] = array(
...
function zen_draw_products_pull_down($name, $parameters = '', $exclude = '', $show_id = false, $set_selected = false, $show_model = false, $show_current_category = false)
{
	...
	if ($show_current_category)
	{
		...
		$products = $db->Execute("select p.products_id, pd.products_name, p.products_price, p.products_model, ptc.categories_id
                                ...
                                left join " . TABLE_PRODUCTS_TO_CATEGORIES . " ptc on ptc.products_id = p.products_id, " . TABLE_PRODUCTS_DESCRIPTION . " pd
...
function zen_get_address_formats()
{
	...
	{
		$address_format_array[] = array(
			'id' => $address_format_values->fields['address_format_id'],
...
function zen_remove_category($category_id)
{
	...
	while(!$chk_sale_categories_selected->EOF)
	{
		...
		if ($chk_sale_categories_selected->fields['sale_categories_selected'] == (int)$category_id and $chk_sale_categories_selected->fields['sale_categories_all'] == ',' . (int)$category_id . ',')
		{ // delete record
		...
		if (!$skip_cats && (preg_match('/^' . (int)$category_id . ',/', $chk_sale_categories_selected->fields['sale_categories_selected'])))
		{ // front
		...
		if (!$skip_cats && (strpos($chk_sale_categories_selected->fields['sale_categories_selected'], ',' . (int)$category_id . ',')))
		{ // middle or end
		...
		if (!$skip_cats && !$skip_cat_last && (strripos($chk_sale_categories_selected->fields['sale_categories_selected'], ',' . (int)$category_id)))
		{ // end
			$start_cat = (int)strpos($chk_sale_categories_selected->fields['sale_categories_selected'], ',' . (int)$category_id) + strlen(',' . (int)$category_id);
		...
		if (!$skip_cats)
		{
			$salemakerupdate = "UPDATE " . TABLE_SALEMAKER_SALES . "
	...
	while(!$chk_sale_categories_all->EOF)
	{
		...
		$skip_cats = false;
		if ($skip_sale_id == $chk_sale_categories_all->fields['sale_id'])
		{ // was deleted
		...
		if (!$skip_cats && (preg_match('/^' . ',' . (int)$category_id . ',/', $chk_sale_categories_all->fields['sale_categories_all'])))
		{ // front
		...
		// if in the middle or end - remove ,9,
		if (!$skip_cats && (strpos($chk_sale_categories_all->fields['sale_categories_all'], ',' . (int)$category_id . ',')))
		{ // middle
		...
		/*
		 * // not needed in loop 2 // if on the end - remove ,9, if (!$skip_cats && (strripos($chk_sale_categories_all->fields['sale_categories_all'], ',' . (int)$category_id . ',')) ) { // end $start_cat = (int)strpos($chk_sale_categories_all->fields['sale_categories_all'], ',' . (int)$category_id) + strlen(',' . (int)$category_id . ','); echo 'D: I need to remove from the end - ,' . (int)$category_id . ', - from the end ' . $chk_sale_categories_all->fields['sale_categories_all'] . '<br>'; $new_sale_categories_all = substr($chk_sale_categories_all->fields['sale_categories_all'], 0, $start_cat - (strlen(',' . (int)$category_id . ',') - 1)); echo 'D: new_sale_categories_all: ' . $new_sale_categories_all. '<br><br>'; }
		 */
...
function zen_validate_options_to_options_value($products_options_id, $products_options_values_id)
{
	...
	$check_options_to_values_query = $db->Execute("select products_options_id
                                                  ...
                                                  and products_options_values_id='" . (int)$products_options_values_id . "' limit 1");
...
function zen_get_products_options_name_from_value($lookup)
{
	...
	$check_options = $db->Execute("select products_options_name
                      from " . TABLE_PRODUCTS_OPTIONS . "
                      where products_options_id='" . (int)$check_options_to_values->fields['products_options_id'] . "' and language_id='" . (int)$_SESSION['languages_id'] . "'");
...
function zen_output_warning($warning)
{
	new errorBox(array(
		array(
			'text' => zen_image(DIR_WS_ICONS . 'warning.gif', ICON_WARNING) . ' ' . $warning 
		) 
	));
}
...
function zen_draw_products_pull_down_attributes($name, $parameters = '', $exclude = '')
{
	...
	$products = $db->Execute("select distinct p.products_id, pd.products_name, p.products_price" . $new_fields . " from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_PRODUCTS_ATTRIBUTES . " pa " . " where p.products_id= pa.products_id and p.products_id = pd.products_id and pd.language_id = '" . (int)$_SESSION['languages_id'] . "' order by products_name");
...
function zen_draw_products_pull_down_categories($name, $parameters = '', $exclude = '', $show_id = false, $show_parent = false)
{
	...
	$categories = $db->Execute("select distinct c.categories_id, cd.categories_name " . " from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd, " . TABLE_PRODUCTS_TO_CATEGORIES . " ptoc " . " where ptoc.categories_id = c.categories_id and c.categories_id = cd.categories_id and cd.language_id = '" . (int)$_SESSION['languages_id'] . "' order by categories_name");
...
function zen_get_product_path($products_id, $status_override = '1')
{
	...
	/*
	 * $category_query = "select p2c.categories_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c where p.products_id = '" . (int)$products_id . "' " . ($status_override == 1 ? " and p.products_status = 1 " : '') . " and p.products_id = p2c.products_id limit 1";
	 */
...
/*
 * //// // Sets the status of a featured product function zen_set_featured_status($featured_id, $status) { global $db; if ($status == '1') { return $db->Execute("update " . TABLE_FEATURED . " set status = '1', expires_date = NULL, date_status_change = NULL where featured_id = '" . (int)$featured_id . "'"); } elseif ($status == '0') { return $db->Execute("update " . TABLE_FEATURED . " set status = '0', date_status_change = now() where featured_id = '" . (int)$featured_id . "'"); } else { return -1; } }
 */
...
function zen_get_master_categories_pulldown($product_id)
{
	...
	{
		$master_category_array[] = array(
			'id' => $master_categories_query->fields['categories_id'],
			'text' => $master_categories_query->fields['categories_name'] . TEXT_INFO_ID . $master_categories_query->fields['categories_id'] 
		);
...
function zen_restrict_sub_categories($zf_cat_id, $zf_type)
{
	...
	while(!$zq_sub_cats->EOF)
	{
		...
		{
			$za_insert_sql_data = array(
				'category_id' => (int)$zq_sub_cats->fields['categories_id'],
...
function zen_get_customers_address_book($customer_id)
{
	...
	$customer_address_book_count_query = "SELECT c.*, ab.* from " . TABLE_CUSTOMERS . " c
...
{
	global $db;
	$orders_comments_query = "SELECT osh.comments from " . TABLE_ORDERS_STATUS_HISTORY . " osh
