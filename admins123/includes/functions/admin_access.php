 ...
 * the page passed as parameter $page, with GET $params .
...
function zen_update_user($name, $email, $id, $profile)
{
	...
	if (sizeof($errors) == 0)
	{
		...
		{
			$changes['email'] = array(
				'old' => $oldData['admin_email'],
				'new' => $newData['admin_email'] 
			);
		}
		if ($oldData['admin_name'] != $newData['admin_name'])
		{
			$changes['name'] = array(
				'old' => $oldData['admin_name'],
				'new' => $newData['admin_name'] 
			);
		}
		if ($oldData['admin_profile'] != $newData['admin_profile'])
		{
			$changes['profile'] = array(
		...
		if ($alertText != '') zen_mail(STORE_OWNER_EMAIL_ADDRESS, STORE_OWNER_EMAIL_ADDRESS, TEXT_EMAIL_SUBJECT_ADMIN_USER_CHANGED, $alertText, STORE_NAME, EMAIL_FROM, array(
			'EMAIL_MESSAGE_HTML' => $alertText,
			'EMAIL_SPAM_DISCLAIMER' => ' ',
			'EMAIL_DISCLAIMER' => ' ' 
		), 'admin_settings_changed');
		if ($alertText != '') zen_mail($oldData['admin_email'], $oldData['admin_email'], TEXT_EMAIL_SUBJECT_ADMIN_USER_CHANGED, $alertText, STORE_NAME, EMAIL_FROM, array(
			'EMAIL_MESSAGE_HTML' => $alertText,
			'EMAIL_SPAM_DISCLAIMER' => ' ',
			'EMAIL_DISCLAIMER' => ' ' 
		), 'admin_settings_changed');
...
function zen_validate_user_login($admin_name, $admin_pass)
{
	...
	else
	{
		...
		if ($error == FALSE && defined('ZC_ADMIN_TWO_FACTOR_AUTHENTICATION_SERVICE') && ZC_ADMIN_TWO_FACTOR_AUTHENTICATION_SERVICE != '')
		{
			...
			{
				$response = zen_call_function(ZC_ADMIN_TWO_FACTOR_AUTHENTICATION_SERVICE, array(
					$result['admin_id'],
					$result['admin_email'],
					$result['admin_name'] 
				));
	...
	if ($error == false)
	{
		...
		$redirect = zen_href_link($camefrom, zen_get_all_get_params(array(
			'camefrom' 
		)), 'SSL');
	}
	return array(
...
function zen_get_profiles($withUsers = FALSE)
{
	...
	if ($withUsers)
	{
		...
		{
			$retVal[] = array(
				'id' => $result->fields['profile_id'],
				'name' => $result->fields['profile_name'],
				'users' => $result->fields['profile_users'] 
			);
			$result->MoveNext();
		}
	}
	else
	{
		...
		while(!$result->EOF)
		{
			$retVal[] = array(
				'id' => $result->fields['profile_id'],
				'text' => $result->fields['profile_name'] 
			);
			$result->MoveNext();
		}
	}
...
function zen_get_admin_pages($menu_only)
{
	...
	while(!$result->EOF)
	{
		$productTypes['_productTypes_' . $result->fields['type_handler']] = array(
			'name' => $result->fields['type_name'],
			'file' => $result->fields['type_handler'],
			'params' => '' 
		);
		$result->MoveNext();
	}
	...
	if (!(defined('MODULE_PAYMENT_PAYPAL_STATUS') && MODULE_PAYMENT_PAYPAL_STATUS == 'True') && !(defined('MODULE_PAYMENT_PAYPALWPP_STATUS') && MODULE_PAYMENT_PAYPALWPP_STATUS == 'True') && !(defined('MODULE_PAYMENT_PAYPALDP_STATUS') && MODULE_PAYMENT_PAYPALDP_STATUS == 'True'))
	{
