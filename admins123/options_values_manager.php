...
if (zen_not_null($action))
{
	...
	switch($action)
	{
		...
		case 'copy_options_values_one_to_another':
			...
			/*
			 * // debug code while(!$products_only->EOF) { echo 'Product ' . $products_only->fields['products_id'] . '<br>'; $products_only->MoveNext(); } die('I SEE match from: ' . $options_id_from . '-' . $options_values_values_id_from . ' add to: ' . $options_id_to . ' -' . $options_values_values_id_to . ' | only for cat ' . $_POST['copy_to_categories_id'] . ' | found matches ' . $products_only->RecordCount());
			 */
		...
		case 'copy_options_values_one_to_another_options_id':
			...
			/*
			 * ///// // debug code while(!$products_only->EOF) { echo 'Product ' . $products_only->fields['products_id'] . '<br>'; $products_only->MoveNext(); } die('I SEE match from products_id:' . $copy_from_products_id . ' options_id_from: ' . $options_id_from . '-' . $options_values_values_id_from . ' add to: ' . $options_id_to . ' | only for cat ' . $_POST['copy_to_categories_id'] . ' | found matches ' . $products_only->RecordCount() . '<br>' . 'from products_id: ' . $products_attributes_defaults->fields['products_id'] . ' option_id: ' . $products_attributes_defaults->fields['options_id'] . ' options_values_id: ' . $products_attributes_defaults->fields['options_values_id'] ); /////
			 */
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="2">
					<tr>
						<td height="40" valign="bottom"><a
							...
							&nbsp; <a
							href="<?php echo  zen_href_link(FILENAME_OPTIONS_NAME_MANAGER, '', 'NONSSL') ?>"><?php echo zen_image_button('button_option_names.gif', IMAGE_OPTION_NAMES); ?></a>
						</td>
...
<?php
		...
		if (($action == 'update_option_value') && ($_GET['value_id'] == $values_values->fields['products_options_values_id']))
		{
			...
			?>
                <td align="center" class="attributeBoxContent">&nbsp;<?php echo $values_values->fields['products_options_values_id']; ?><input
				type="hidden" name="value_id"
				value="<?php echo $values_values->fields['products_options_values_id']; ?>">&nbsp;
			</td>
...
<?php
		...
		?>
			...
			<td class="smallText"><input type="hidden" name="value_id"
				value="<?php echo $next_id; ?>"><?php echo $inputs; ?></td>
			<td colspan="1" class="smallText"><input type="hidden"
				name="value_id" value="<?php echo $next_id; ?>"><?php echo $inputs2; ?></td>
...
<?php
	...
	/*
	 * this builds the resulting values for use in the case statements above $options_id_from = $_POST['options_id_from']; $options_values_values_id_from = $_POST['options_values_values_id_from']; $options_id_to = $_POST['options_id_to']; $options_values_values_id_to = $_POST['options_values_values_id_to'];
	 */
...
-->
		...
		<tr class="dataTableHeadingRow">
			<td colspan="5"><table border="1" cellspacing="0" cellpadding="2"
					width="100%">
					<tr class="dataTableHeadingRow">
						<form name="quick_jump" method="post"
							action="<?php echo zen_href_link(FILENAME_OPTIONS_VALUES_MANAGER, 'action=copy_options_values_one_to_another', 'NONSSL'); ?>"><?php echo zen_draw_hidden_field('securityToken', $_SESSION['securityToken']); ?>
                  <td class="dataTableHeadingContent">
                  <?php
	
echo TEXT_SELECT_OPTION_FROM . '<br />' . $option_from_dropdown . '&nbsp;<br />' . TEXT_SELECT_OPTION_VALUES_FROM . '<br />' . $option_values_from_dropdown;
	?>&nbsp;
                  </td>
							<td class="dataTableHeadingContent">
                  <?php
	
echo TEXT_SELECT_OPTION_TO . '<br />' . $option_to_dropdown . '&nbsp<br />' . TEXT_SELECT_OPTION_VALUES_TO . '<br />' . $option_values_to_dropdown;
	?>&nbsp;
                  </td>
...
-->
		...
		<tr class="dataTableHeadingRow">
			<td colspan="5"><table border="1" cellspacing="0" cellpadding="2"
					...
					<tr class="dataTableHeadingRow">
						<form name="quick_jump" method="post"
							action="<?php echo zen_href_link(FILENAME_OPTIONS_VALUES_MANAGER, 'action=delete_options_values_of_option_name', 'NONSSL'); ?>"><?php echo zen_draw_hidden_field('securityToken', $_SESSION['securityToken']); ?>
                  <td class="dataTableHeadingContent">
                  <?php
	
echo TEXT_SELECT_DELETE_OPTION_FROM . '<br />' . $option_from_dropdown . '&nbsp;<br />' . TEXT_SELECT_DELETE_OPTION_VALUES_FROM . '<br />' . $option_values_from_dropdown;
	?>&nbsp;
                  </td>
							<td class="dataTableHeadingContent"><?php echo $to_categories_id; ?>&nbsp;</td>
...
-->
		...
		<tr class="dataTableHeadingRow">
			<td colspan="5"><table border="1" cellspacing="0" cellpadding="2"
					...
					<tr class="dataTableHeadingRow">
						<form name="quick_jump" method="post"
							action="<?php echo zen_href_link(FILENAME_OPTIONS_VALUES_MANAGER, 'action=copy_options_values_one_to_another_options_id', 'NONSSL'); ?>"><?php echo zen_draw_hidden_field('securityToken', $_SESSION['securityToken']); ?>
