			...
			switch(true)
			{
				...
				case (substr($line_upper, 0, 13) == 'CREATE INDEX ' || (strtoupper($param[0]) == 'CREATE' && strtoupper($param[2]) == 'INDEX')):
					...
					else
					{
						...
						{
							$line = trim('CREATE INDEX ' . $param[2] . ' ON ' . $table_prefix . implode(' ', array(
								$param[4],
								$param[5],
								$param[6],
								$param[7],
						...
						{
							$line = trim('CREATE ' . $param[1] . ' INDEX ' . $param[3] . ' ON ' . $table_prefix . implode(' ', array(
								$param[5],
								$param[6],
								$param[7],
								$param[8],
...
function zen_check_database_privs($priv = '', $table = '', $show_privs = false)
{
	...
	while(!$result->EOF)
	{
		...
		$granted_db = str_replace(array(
			'`',
			'\\' 
		), '', substr($granted_privs, strpos($granted_privs, ' ON ') + 4)); // remove backquote and find "ON" string
...
function zen_create_index_command($param)
{
	...
	/*
	 * @TODO: verify that individual columns exist, by parsing the index_col_name parameters list Structure is (colname(len)), or (colname),
	 */
}
function zen_check_alter_command($param)
{
	...
	switch(strtoupper($param[3]))
	{
		case ("ADD"):
			...
			elseif (!in_array(strtoupper($param[4]), array(
				'CONSTRAINT',
				'UNIQUE',
				'PRIMARY',
				'FULLTEXT',
	...
	<table border="0" width="100%" cellspacing="2" cellpadding="2">
		<tr>
			...
			<td width="100%" valign="top"><table border="0" width="100%"
					...
					<tr>
						<td class="alert"><?php echo HEADING_WARNING; ?></td>
					</tr>
					<tr>
...
<?php
    ...
    <tr><?php echo zen_draw_form('getquery', FILENAME_SQLPATCH, 'action=execute' . (($debug==true)?'&debug=ON':'') . (($skip_stripslashes==true)?'&keepslashes=1':''),'post', ''); ?>
      <td><table border="0" cellpadding="0" cellspacing="2">
		...
		<tr>
			<td class="alert" colspan="2" style="padding-left: 110px;"><?php echo HEADING_WARNING_INSTALLSCRIPTS; ?></td>
		</tr>
      ...
      <td><table border="0" cellpadding="0" cellspacing="2">
		...
		<tr>
			<td width="300" align="right"><a
				href="<?php echo zen_href_link(FILENAME_SQLPATCH, 'action=help'); ?>"
				target='_blank'><?php echo zen_image_button('button_details.gif', IMAGE_DETAILS); ?></a></td>
		</tr>
		<!-- body_text_eof //-->
	</table>
	<!-- body_eof //-->
