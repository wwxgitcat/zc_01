...
<?php
	if ($cr_list->fields['coupon_restrict'] == 'N')
	{
		echo '<td class="dataTableContent" align="center"><a href="' . zen_href_link('coupon_restrict.php', zen_get_all_get_params(array(
			...
			'x',
			'y' 
		)) . 'action=switch_status&info=' . $cr_list->fields['restrict_id'], 'NONSSL') . '" onClick="divertClickSwitchStatus(this.href);return false;" >' . zen_image(DIR_WS_IMAGES . 'icon_status_green.gif', IMAGE_ALLOW) . '</a></td>';
	}
	else
	{
		echo '<td class="dataTableContent" align="center"><a href="' . zen_href_link('coupon_restrict.php', zen_get_all_get_params(array(
			...
			'x',
			'y' 
		)) . 'action=switch_status&info=' . $cr_list->fields['restrict_id'], 'NONSSL') . '" onClick="divertClickSwitchStatus(this.href);return false;" >' . zen_image(DIR_WS_IMAGES . 'icon_status_red.gif', IMAGE_DENY) . '</a></td>';
	}
	...
	echo '<td class="dataTableContent" align="center"><a href="' . zen_href_link('coupon_restrict.php', zen_get_all_get_params(array(
		...
		'x',
		'y' 
	)) . 'action=remove&info=' . $cr_list->fields['restrict_id'], 'NONSSL') . '" onClick="divertClickActionRemove(this.href);return false;" >' . zen_image(DIR_WS_IMAGES . 'icons/delete.gif', IMAGE_REMOVE) . '</a></td>';
	?>
              </tr>
<?php
	$cr_list->MoveNext();
...
?>
              <tr>
														...
														<tr>
															<form name="restrict_category" method="post"
																action="<?php echo zen_href_link('coupon_restrict.php', zen_get_all_get_params(array('info', 'action', 'x', 'y')) . 'action=add_category&info=' . $cInfo->restrict_id, 'NONSSL'); ?>"><?php echo zen_draw_hidden_field('securityToken', $_SESSION['securityToken']); ?>
                <td colspan="7"><table border="0" width="100%"
																		cellspacing="0" cellpadding="2">
																		<tr>
																			...
																			<td class="smallText" align="left"><input
																				type="submit" name="add" value="Add"></td>
																			<td class="smallText" align="left">&nbsp;</td>
																			<td class="smallText" align="left">&nbsp;</td>
																		</tr>
																	</table></td>
...
<?php
	if ($pr_list->fields['coupon_restrict'] == 'N')
	{
		echo '<td class="dataTableContent" align="center"><a href="' . zen_href_link('coupon_restrict.php', zen_get_all_get_params(array(
			...
			'x',
			'y' 
		)) . 'action=switch_status&info=' . $pr_list->fields['restrict_id'], 'NONSSL') . '" onClick="divertClickSwitchStatus(this.href);return false;" >' . zen_image(DIR_WS_IMAGES . 'icon_status_green.gif', IMAGE_ALLOW) . '</a></td>';
	}
	else
	{
		echo '<td class="dataTableContent" align="center"><a href="' . zen_href_link('coupon_restrict.php', zen_get_all_get_params(array(
			...
			'x',
			'y' 
		)) . 'action=switch_status&info=' . $pr_list->fields['restrict_id'], 'NONSSL') . '" onClick="divertClickSwitchStatus(this.href);return false;" >' . zen_image(DIR_WS_IMAGES . 'icon_status_red.gif', IMAGE_DENY) . '</a></td>';
	}
	if ($pr_list->fields['coupon_restrict'] == 'Y')
	{
		echo '<td class="dataTableContent" align="center"><a href="' . zen_href_link('coupon_restrict.php', zen_get_all_get_params(array(
			...
			'x',
			'y' 
		)) . 'action=switch_status&info=' . $pr_list->fields['restrict_id'], 'NONSSL') . '" onClick="divertClickSwitchStatus(this.href);return false;" >' . zen_image(DIR_WS_IMAGES . 'icon_status_green.gif', IMAGE_DENY) . '</a></td>';
	}
	else
	{
		echo '<td class="dataTableContent" align="center"><a href="' . zen_href_link('coupon_restrict.php', zen_get_all_get_params(array(
			...
			'x',
			'y' 
		)) . 'action=switch_status&info=' . $pr_list->fields['restrict_id'], 'NONSSL') . '" onClick="divertClickSwitchStatus(this.href);return false;" >' . zen_image(DIR_WS_IMAGES . 'icon_status_red.gif', IMAGE_ALLOW) . '</a></td>';
	}
	echo '<td class="dataTableContent" align="center"><a href="' . zen_href_link('coupon_restrict.php', zen_get_all_get_params(array(
		'info',
		'action',
		'x',
		'y' 
	)) . 'action=remove&info=' . $pr_list->fields['restrict_id'], 'NONSSL') . '" onClick="divertClickActionRemove(this.href);return false;" >' . zen_image(DIR_WS_IMAGES . 'icons/delete.gif', IMAGE_REMOVE) . '</a></td>';
	?>
              </tr>
<?php
	$pr_list->MoveNext();
...
$products = $db->Execute("select p.products_id, pd.products_name from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c
...
?>
                    <td class="smallText" valign="top"><?php echo HEADER_CATEGORY_NAME; ?></td>
																			<td class="smallText" align="left"></td>
																			<form name="restrict_product" method="post"
																				action="<?php echo zen_href_link('coupon_restrict.php', zen_get_all_get_params(array('info', 'action', 'x', 'y')) . 'info=' . $cInfo->restrict_id, 'NONSSL'); ?>">
                    <?php echo zen_hide_session_id(); ?>
                    <td class="smallText" align="left"><?php echo zen_draw_pull_down_menu('cPath_prod', zen_get_category_tree(), $current_category_id, 'onChange="this.form.submit();"'); ?></td>
																			</form>
<?php if (sizeof($products_array) > 0) { ?>
                    <form name="restrict_category" method="post"
																				action="<?php echo zen_href_link('coupon_restrict.php', zen_get_all_get_params(array('info', 'action', 'x', 'y')) . 'action=add_product&info=' . $cInfo->restrict_id . '&build_cat=' . $current_category_id, 'NONSSL'); ?>"><?php echo zen_draw_hidden_field('securityToken', $_SESSION['securityToken']); ?>
                    <td class="smallText" valign="top"><?php echo HEADER_PRODUCT_NAME; ?></td>
																				...
																				<td class="smallText" align="left"><input
																					type="submit" name="add" value="Update"></td>
																				<td class="smallText" align="left">&nbsp;</td>
																				<td class="smallText" align="left">&nbsp;</td>
<?php } else { ?>
...
<!-- footer_eof //-->
	...
	<form name="actionRemove" id="actionRemove" action="#" method="post">
		<input type="hidden" name="securityToken"
			value="<?php echo $_SESSION['securityToken']; ?>" /> <input
			type="hidden" name="actionRemoveProto" value="" />
	</form>
	<form name="switchStatus" id="switchStatus" action="#" method="post">
		<input type="hidden" name="securityToken"
			value="<?php echo $_SESSION['securityToken']; ?>" /> <input
			type="hidden" name="switchStatusProto" value="" />
	</form>
	<script type="text/javascript">
