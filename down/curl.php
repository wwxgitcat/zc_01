...
class CURL
{
	...
	private $failedSaveFile;
	
	public function __construct()
	{
		$this->failedSaveFile = dirname(__FILE__).'/failed.links';
	}
	/**
	 * running infomation
	 */
	function status($debug = false)
	{
		...
		else
		{
			...
			if ($now > $last or ($this->finishNum == $this->taskNum))
			{
				...
				if ($this->finishNum == $this->taskNum) echo "\n";
			}
		}
	}
	...
	function read($url)
	{
		...
		if (curl_errno($ch) === 0)
		{
			...
			$r['content'] = $content;
			if ($this->cache['on']) $this->cache($url, $r);
		}
		else
		{
	 ...
	 *
	 * @param array $url
	 *        	$url[0] is url,$url[1] is file path if isset
	 * @param array $p
	 *        	success callback,$p[0] is callback,$p[1] is param for the callback,$p[2] is curl option
	 * @param array $f
	 *        	fail callback,$f[0] is callback,$f[1] is param for the callback
	 */
	function add($url = array(), $p = array(), $f = array())
	{
		...
		if (!is_array($p) or !is_array($f)) user_error('callback is not array', E_USER_ERROR);
		if (!isset($p[0])) user_error('process callback is not set', E_USER_ERROR);
		...
		// fix
		if (empty($url[1])) $url[1] = null;
		if (!isset($p[1])) $p[1] = array();
		if (isset($f[0]) and !isset($f[1])) $f[1] = array();
		if (empty($p[2])) $p[2] = null;
		$task = array();
		$task[self::ITEM_URL] = $url;
		$task[self::ITEM_P] = $p;
		$task[self::ITEM_P_OPT] = $p[2];
		$task[self::ITEM_F] = $f;
	...
	function go()
	{
		...
		if ($running) user_error('CURL can only run one instance', E_USER_ERROR);
			...
			while($curlInfo = curl_multi_info_read($this->mh, $this->queueNum))
			{
				...
				if ($curlInfo['result'] == CURLE_OK)
				{
					if (isset($task[self::ITEM_P]))
					{
						$param = array();
						$param['info'] = $info;
						if (!isset($task[self::ITEM_URL][1])) $param['content'] = curl_multi_getcontent($ch);
						array_unshift($task[self::ITEM_P][1], $param);
					}
					// write cache
					if ($this->cache['on'] and !isset($task[self::ITEM_URL][1])) $this->cache($task[self::ITEM_URL][0], $param);
				}
				else
				{
					if ($task[self::ITEM_TRYED] >= $this->maxTry)
					{
						...
						$this->failedFile($task[self::ITEM_URL]);
						$this->failedNum++;
					}
					else
					{
				...
				curl_close($ch);
				if (isset($task[self::ITEM_FP])) fclose($task[self::ITEM_FP]);
	...
	private function failedFile($info)
	{
		if (!is_array($info))
			$info = (array)$info;
		file_put_contents($this->failedSaveFile, implode(',', $info)."\n", FILE_APPEND);
	}
	/**
	 * add a task to curl
	 */
			...
			else
			{
				if (0 < $left = (int)($this->limit - count($this->taskPool)) and isset($this->task))
				{
					while($left-- > 0)
					{
						...
						if (count($this->taskPool) >= $this->limit) break;
					}
				}
				if (!empty($this->taskPool)) $task = array_pop($this->taskPool);
			}
			$cache = null;
			if (!empty($task))
			{
	...
	{
		$key = md5($url);
		if (!isset($this->cache['dir'])) user_error('Cache dir is not defined', E_USER_ERROR);
		...
		else
		{
			...
			{
				$dir = dirname($file);
				if (!file_exists($dir) and !mkdir($dir, 0777, true)) user_error("Create dir failed", E_USER_WARNING);
				$content = serialize($content);
				if (file_put_contents($file, $content, LOCK_EX)) $r = true;
				else
	...
	private function init($url)
	{
		...
		// user defined opt
		if (!empty($this->opt)) foreach($this->opt as $k => $v)
			$opt[$k] = $v;
