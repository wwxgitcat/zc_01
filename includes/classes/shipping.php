...
class shipping extends base
{
	...
	function shipping($module = '')
	{
		...
		if (defined('MODULE_SHIPPING_INSTALLED') && zen_not_null(MODULE_SHIPPING_INSTALLED))
		{
			...
			if ((zen_not_null($module)) && isset($module['id']) && (in_array(substr($module['id'], 0, strpos($module['id'], '_')) . '.' . substr($PHP_SELF, (strrpos($PHP_SELF, '.') + 1)), $this->modules)))
			{
				$include_modules[] = array(
	...
	function calculate_boxes_weight_and_tare()
	{
		...
		if (is_array($this->modules))
		{
			...
			/*
			 * if (SHIPPING_BOX_WEIGHT >= $shipping_weight*SHIPPING_BOX_PADDING/100) { $shipping_weight = $shipping_weight+SHIPPING_BOX_WEIGHT; } else { $shipping_weight = $shipping_weight + ($shipping_weight*SHIPPING_BOX_PADDING/100); }
			 */
