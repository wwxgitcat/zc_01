...
{
	var $info, $totals, $products, $customer, $delivery, $content_type, $email_low_stock, $products_ordered_attributes, $products_ordered, $products_ordered_email, $attachArray;
	...
	function cart()
	{
		...
		/*
		 * // this is set above to the module filename it should be set to the module title like Checks/Money Order rather than moneyorder if (isset($$_SESSION['payment']) && is_object($$_SESSION['payment'])) { $this->info['payment_method'] = $$_SESSION['payment']->title; }
		 */
...
*/
    $this->customer = array(
		...
		for($i = 0, $n = sizeof($products); $i < $n; $i++)
		{
			...
			$this->notify('NOTIFY_ORDER_CART_ADD_PRODUCT_LIST', array(
				'index' => $index,
				'products' => $products[$i] 
			));
			if ($products[$i]['attributes'])
			{
				...
				{
					/*
					 * //clr 030714 Determine if attribute is a text attribute and change products array if it is. if ($value == PRODUCTS_OPTIONS_VALUES_TEXT_ID){ $attr_value = $products[$i]['attributes_values'][$option]; } else { $attr_value = $attributes->fields['products_options_values_name']; }
					 */
					...
					$this->notify('NOTIFY_ORDER_CART_ADD_ATTRIBUTE_LIST', array(
						'index' => $index,
						'subindex' => $subindex,
						'products' => $products[$i],
			...
			$this->notify('NOTIFIY_ORDER_CART_SUBTOTAL_CALCULATE', array(
		...
		/*
		 * // moved to function create if ($this->info['total'] == 0) { if (DEFAULT_ZERO_BALANCE_ORDERS_STATUS_ID == 0) { $this->info['order_status'] = DEFAULT_ORDERS_STATUS_ID; } else { $this->info['order_status'] = DEFAULT_ZERO_BALANCE_ORDERS_STATUS_ID; } }
		 */
		if (isset($GLOBALS[$class]) && is_object($GLOBALS[$class]))
		{
			if (isset($GLOBALS[$class]->order_status) && is_numeric($GLOBALS[$class]->order_status) && ($GLOBALS[$class]->order_status > 0))
	...
	function create($zf_ot_modules, $zf_mode = 2)
	{
		...
		$this->notify('NOTIFY_ORDER_DURING_CREATE_ADDED_ORDER_HEADER', array_merge(array(
			'orders_id' => $insert_id,
	...
	function create_add_products($zf_insert_id, $zf_mode = false)
	{
		...
		for($i = 0, $n = sizeof($this->products); $i < $n; $i++)
		{
			...
			$this->notify('NOTIFY_ORDER_DURING_CREATE_ADDED_PRODUCT_LINE_ITEM', array_merge(array(
			...
			/*
			 * START: ADD MY CUSTOM DETAILS 1. calculate/prepare custom information to be added to this product entry in order-confirmation, perhaps as a function call to custom code to build a serial number etc: Possible parameters to pass to custom functions at this point: Product ID ordered (for this line item): $this->products[$i]['id'] Quantity ordered (of this line-item): $this->products[$i]['qty'] Order number: $zf_insert_id Attribute Option Name ID: (int)$this->products[$i]['attributes'][$j]['option_id'] Attribute Option Value ID: (int)$this->products[$i]['attributes'][$j]['value_id'] Attribute Filename: $attributes_values->fields['products_attributes_filename'] 2. Add that data to the $this->products_ordered_attributes variable, using this sort of format: $this->products_ordered_attributes .= {INSERT CUSTOM INFORMATION HERE};
			 */
	...
	function send_order_email($zf_insert_id, $zf_mode)
	{
		...
		// $html_msg['EMAIL_TEXT_HEADER'] = EMAIL_TEXT_HEADER;
		$html_msg['EXTRA_INFO'] = '';
		$this->notify('NOTIFY_ORDER_INVOICE_CONTENT_READY_TO_SEND', array(
		...
		if (SEND_EXTRA_ORDER_EMAILS_TO != '')
		{
			...
			$this->notify('NOTIFY_ORDER_INVOICE_CONTENT_FOR_ADDITIONAL_EMAILS', array(
				'zf_insert_id' => $zf_insert_id,
				'text_email' => $email_order,
				'html_email' => $html_msg 
			));
			zen_mail('', SEND_EXTRA_ORDER_EMAILS_TO, SEND_EXTRA_NEW_ORDERS_EMAILS_TO_SUBJECT . ' ' . EMAIL_TEXT_SUBJECT . EMAIL_ORDER_NUMBER_SUBJECT . $zf_insert_id, $email_order . $extra_info['TEXT'], STORE_NAME, EMAIL_FROM, $html_msg, 'checkout_extra', $this->attachArray);
		}
		$this->notify('NOTIFY_ORDER_AFTER_SEND_ORDER_EMAIL', array(
			$zf_insert_id,
			$email_order,
			$extra_info,
			$html_msg 
