...
class zen_SiteMapXML
{
	...
	function zen_SiteMapXML($inline = false, $ping = false, $rebuild = false, $genxml = true)
	{
		...
		/*
		 * if (!($robots_txt = @file_get_contents($this->savepath . 'robots.txt'))) { $this->message('<b>File "robots.txt" not found in save path - "' . $this->savepath . 'robots.txt"</b>' . '<br />'); } elseif (!preg_match("@Sitemap:\s*(.*)\n@i", $robots_txt . "\n", $m)) { $this->message('<b>Sitemap location don\'t specify in robots.txt</b>' . '<br />'); } elseif (trim($m[1]) != $this->base_url . $this->sitemapindex) { $this->message('<b>Sitemap location specified in robots.txt "' . trim($m[1]) . '" another than "' . $this->base_url . $this->sitemapindex . '"</b>' . '<br />'); }
		 */
	}
	...
	function _checkFTimeSitemap($filename, $last_date = 0)
	{
		...
		clearstatcache();
		if (SITEMAPXML_USE_EXISTING_FILES == 'true' && file_exists($this->savepath . $filename) && (filemtime($this->savepath . $filename) >= strtotime($last_date)) && filesize($this->savepath . $filename) > 0)
		{
