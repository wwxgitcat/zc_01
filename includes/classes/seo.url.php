<?php
/*
 * +----------------------------------------------------------------------+ |	Ultimate SEO URLs For Zen Cart, version 2.101 | +----------------------------------------------------------------------+ | | |	Derrived from Ultimate SEO URLs v2.1 for osCommerce by Chemo | | | |	Portions Copyright 2011, Andrew Ballanger | | | |	Portions Copyright 2005, Joshua Dechant | | | |	Portions Copyright 2005, Bobby Easland | | | |	Portions Copyright 2003 The zen-cart developers | | | +----------------------------------------------------------------------+ | This source file is subject to version 2.0 of the GPL license, | | that is bundled with this package in the file LICENSE, and is | | available through the world-wide-web at the following url: | | http://www.zen-cart.com/license/2_0.txt. | | If you did not receive a copy of the zen-cart license and are unable | | to obtain it through the world-wide-web, please send a note to | | license@zen-cart.com so we can mail you a copy immediately. | @author JUns +----------------------------------------------------------------------+
 */
...
class SEO_URL
{
			...
			switch($p2[0])
			{
				...
				case 'products_id':
					switch(true)
					{
						...
						// !$this->is_attribute_string($params)):
						case ($page == FILENAME_PRODUCT_INFO):
	 ...
	 * @param string $page        	
	 * @param string $string
	 *        	Stripped, formed anchor
	 * @param string $anchor_type
	 *        	Parameter type (products_id, cPath, etc.)
	 * @param integer $id        	
	 ...
	 * Use evaluated cache, per page cache, or database query in that order of
	 * precedent
	 *
	 * @author Bobby Easland
	 * @version 1.1
	...
	function get_product_name($pID)
	{
		switch(true)
		{
			...
			default:
				...
				if ($this->attributes['SEO_ADD_PRODUCT_CAT'] == 'true')
				{
					$sql = 'SELECT pd.products_name AS pName, ptc.categories_id AS c_id ' . 'FROM ' . TABLE_PRODUCTS_DESCRIPTION . ' AS pd ' . 'LEFT JOIN ' . TABLE_PRODUCTS . ' AS p ' . 'ON pd.products_id=p.products_id ' . 'LEFT JOIN ' . TABLE_PRODUCTS_TO_CATEGORIES . ' AS ptc ' . 'ON pd.products_id=ptc.products_id ' . 'WHERE pd.products_id=\'' . (int)$pID . '\' ' . 'AND language_id=\'' . (int)$this->languages_id . '\' LIMIT 1';
				}
				else
				{
					// END SEO-ADD-PRODUCT-CAT PATCH
					$sql = 'SELECT pd.products_name as pName ' . 'FROM ' . TABLE_PRODUCTS_DESCRIPTION . ' pd ' . 'WHERE products_id=\'' . (int)$pID . '\' ' . 'AND language_id=\'' . (int)$this->languages_id . '\' LIMIT 1';
					// START SEO-ADD-PRODUCT-CAT PATCH
				}
				// END SEO-ADD-PRODUCT-CAT PATCH
				$result = $this->db->Execute($sql, false, true, 43200);
	...
	{
 		$full_cPath = $this->get_full_cPath($cID, $single_cID); // full cPath
		                                                        // needed for
		                                                        // uniformity
		switch(true)
		{
	...
	/**
	 * Function to get the EZ-Pages name.
	 * Use evaluated cache, per page cache, or database query in that order of
	 * precedent.
	 *
	 ...
	 * @version 1.1
	 * @param mixed $cID
	 *        	Could contain cPath or single category_id
	 * @param integer $original
	 *        	Single category_id passed back by reference
	...
	{
		$sql = 'SELECT c.parent_id AS p_id, cd.categories_name AS name ' . 'FROM ' . TABLE_CATEGORIES . ' c ' . 'LEFT JOIN ' . TABLE_CATEGORIES_DESCRIPTION . ' cd ' . 'ON c.categories_id=cd.categories_id ' . 'AND cd.language_id=\'' . (int)$this->languages_id . '\'' . 'WHERE c.categories_id=\'' . (int)$categories_id . '\'';
		...
		while(!$parent->EOF)
		{
			...
			// Recurse if the parent id is not empty or equal the passed
			// categories id
			...
			$path[sizeof($path)] = $this->strip($parent->fields['name']) . '-c-' . (sizeof($cPath) > 1 ? implode('_', $cPath) : $categories_id);
	...
	function strip($string)
	{
		...
		{
			$retval = preg_replace(array_keys($this->attributes['SEO_URLS_FILTER_PCRE']), array_values($this->attributes['SEO_URLS_FILTER_PCRE']), $retval);
		}
		// END SEO_URLS_FILTER_PCRE PATCH
		...
		//$retval = preg_replace($pattern, '', GBsubstr($retval, 0, 96));
		$retval = preg_replace($pattern, '', $this->cut_str($retval, 64));
		// $retval = preg_replace('/\s/', '-', $retval);
		$retval = str_replace(" ", "-", $retval); // 替换英文空格
		$retval = str_replace(chr(32), "-", $retval); // 替换中文空格
		$retval = str_replace('　', "-", $retval); // 替换utf-8空格
	 ...
	 * @param array $parse
	 *        	Array of tarnslation variables
	 * @return string
	 */
	 ...
	 *        	Array of translation characters
	 * @param boolean $protected
	 *        	Switch for htemlspecialchars processing
	 * @return string
	 */
	...
	function generate_products_cache()
	{
		...
		if (!$is_cached || $is_expired)
		{
			 ...
			 */
			$sql = 'SELECT p.products_id as id, ptc.categories_id as c_id, pd.products_name as name ' . 'FROM ' . TABLE_PRODUCTS . ' AS p ' . 'LEFT JOIN ' . TABLE_PRODUCTS_DESCRIPTION . ' AS pd ' . 'ON p.products_id=pd.products_id ' . 'LEFT JOIN ' . TABLE_PRODUCTS_TO_CATEGORIES . ' AS ptc ' . 'ON p.products_id=ptc.products_id ' . 'WHERE p.products_status=\'1\' ' . 'AND pd.language_id=\'' . (int)$this->languages_id . '\'';
			// END SEO-ADD-PRODUCT-CAT PATCH
	...
	function generate_categories_cache()
	{
		...
		{ // it's not cached so create it
			switch(true)
			{
				 ...
				 * START SEO-ADD-PRODUCT-CAT PATCH Patched to use SEO_ADD_PRODUCT_CAT This allows the use of a directory structure for urls. @author Andrew Ballanger
				 */
				// Disable SEO_ADD_CAT_PARENT if using the SEO-ADD-PRODUCT-CAT
				// patch
	 ...
	 *        	Cache name
	 * @param mixed $value
	 *        	Can be array, string, PHP code, or just about anything
	 * @param string $method
	 *        	RETURN, ARRAY, EVAL
	 * @param integer $gzip
	 *        	Enables compression
	 * @param integer $global
	 *        	Sets whether cache record is global is scope
	...
	function save_cache($name, $value, $method = 'RETURN', $gzip = 1, $global = 0, $expires = '30 days')
	{
		...
		if (isset($queryCache) && $queryCache->inCache($query)) $queryCache->removeFromCache($query);
	...
	function get_cache($name = 'GLOBAL', $local_memory = false)
	{
		...
		$global = ($name == 'GLOBAL' ? true : false); // was GLOBAL passed or is
		                                              // using the default?
		switch($name)
		{
			case 'GLOBAL':
	...
	function need_redirect()
	{
		...
		// QUICK AND DIRTY WAY TO DISABLE REDIRECTS ON PAGES WHEN
		// SEO_URLS_ONLY_IN is enabled IMAGINADW.COM
	 ...
	 * @return string
	 */
	function cut_str($str, $length, $more = '')
	{
		$return = '';
		$i = 0;
		$n = 0;
