...
class ih_image
{
	 ...
	 * @version 1.99
	 * @param string $src
	 *        	Image source (e.g. - images/productimage.jpg)
	 * @param string $width
	 *        	The image's width
	 * @param string $height
	 *        	The image's height
	...
	function resizing_allowed()
	{
		...
		$allowed = false;
		if ($ihConf['resize'] && ((strpos($this->src, $ihConf['dir']['images']) === 0) || ((strpos($this->src, substr($bmzConf['cachedir'], strlen($ihConf['dir']['docroot']))) === 0))) && (strpos($this->src, $ihConf['noresize_key']) === false))
		{
			$allowed = true;
