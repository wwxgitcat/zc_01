...
class phpBB extends base
{
	...
	function phpBB()
	{
		...
		{ // if disabled in Zen Cart admin, don't do any checks for phpBB
			$this->get_phpBB_info();
	...
	function get_phpBB_info()
	{
		...
		$this->dir_phpbb = str_replace(array(
			'\\',
			'//' 
		), '/', DIR_WS_PHPBB); // convert slashes
		...
		if (@file_exists($this->dir_phpbb . 'config.php'))
		{
			...
			{ // read the configure.php file for specific variables
	...
	function set_status()
	{
		...
		$script_filename = str_replace(array(
			'\\',
			'//' 
		), '/', $script_filename); // convert slashes
		...
		if (($this->phpBB['db_installed']) && ($this->phpBB['files_installed']) && (PHPBB_LINKS_ENABLED == 'true'))
		{
			...
			$this->phpBB['phpbb_url'] = str_replace(array(
				$_SERVER['DOCUMENT_ROOT'],
				substr($script_filename, 0, strpos($script_filename, $_SERVER['PHP_SELF'])) 
			), '', $this->phpBB['phpbb_path']);
