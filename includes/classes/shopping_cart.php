		...
		while(list($products_id, ) = each($this->contents))
		{
			...
			if (isset($this->contents[$products_id]['attributes']))
			{
				...
				while(list($option, $value) = each($this->contents[$products_id]['attributes']))
				{
					$adjust_downloads++;
					/*
					 * products_attributes_id, options_values_price, price_prefix, attributes_display_only, product_attribute_is_free, attributes_discounted
					 */
	...
	function attributes_price($products_id)
	{
		...
		if (isset($this->contents[$products_id]['attributes']))
		{
			...
			while(list($option, $value) = each($this->contents[$products_id]['attributes']))
			{
				...
				/*
				 * //// extra testing not required on text attribute this is done in application_top before it gets to the cart if ($attribute_price->fields['attributes_required']) { $_SESSION['valid_to_checkout'] = false; $_SESSION['cart_errors'] .= zen_get_products_name($attribute_price->fields['products_id'], $_SESSION['languages_id']) . ERROR_PRODUCT_OPTION_SELECTION . '<br />'; }
				 */
			}
		}
	...
	function get_products($check_for_valid_cart = false)
	{
		...
		while(list($products_id, ) = each($this->contents))
		{
			...
			if ($products = $db->Execute($products_query))
			{
				...
				// fix here
				/*
				 * $special_price = zen_get_products_special_price($prid); if ($special_price) { $products_price = $special_price; }
				 */
				$special_price = zen_get_products_special_price($prid);
