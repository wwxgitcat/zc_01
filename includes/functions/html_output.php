...
/*
 * The HTML image wrapper function for non-proportional images used when "proportional images" is turned off or if calling from a template directory
 */
 ...
 */
function zen_image($src, $alt = '', $width = '', $height = '', $parameters = '', $justPath = false)
{
	...
	{
		$img = zen_image_OLD($src, $alt, $width, $height, $parameters);
		if (!$justPath)
			return $img;
		
		$img = str_ireplace('<img src="', '', $img);
	...
	if (function_exists('handle_image'))
	{
		...
		$zco_notifier->notify('NOTIFY_HANDLE_IMAGE', array(
			$newimg 
		));
	}
...
/*
 * The HTML form submit button wrapper function Outputs a "submit" button in the selected language
 */
...
/*
 * Hide form elements while including session id info IMPORTANT: This should be used in every FORM that has an OnSubmit() function tied to it, to prevent unexpected logouts
 */
...
/*
 * Output a form pull down menu Pulls values from a passed array, with the indicated option pre-selected
 */
...
function zen_get_country_list($name, $selected = '', $parameters = '')
{
	...
	for($i = 0, $n = sizeof($countries); $i < $n; $i++)
	{
		...
		if (!$alreadyInList) $countries_array[] = array(
...
function zen_href_params($page = '', $parameters = '')
{
	...
	if (defined('ROBOTS_PAGES_TO_SKIP') && in_array($page, explode(",", constant('ROBOTS_PAGES_TO_SKIP'))) || $current_page_base == 'down_for_maintenance') $addparms = 'rel="nofollow"';
