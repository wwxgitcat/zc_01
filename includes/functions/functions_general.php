 ...
 *        	string The string to be parsed
 * @param
 *        	string contains a string to be translated, otherwise just quote is translated
 * @param
 *        	boolean Do we run htmlspecialchars over the string
 *        	
 */
 ...
 *
 * @param
 *        	string The string to be broken up
 * @param
 *        	int The maximum length allowed
 * @param
 *        	string The character to use at the end of the broken line
 *        	
 */
 ...
 *
 * @param
 *        	mixed either a single or array of parameter names to be excluded from output
 *        	
 */
...
function zen_parse_search_string($search_str = '', &$objects)
{
	...
	for($k = 0; $k < count($pieces); $k++)
	{
		...
		{
			/*
			 * This means that the $piece is either the beginning or the end of a string. So, we'll slurp up the $pieces and stick them together until we get to the end of the string or run out of pieces.
			 */
			...
			while(($flag == 'on') && ($k < count($pieces)))
			{
				...
				{
					/*
					 * If the $piece ends in double quotes, strip the double quotes, tack the $piece onto the tail of the string, push the $tmpstring onto the $haves, kill the $tmpstring, turn the $flag "off", and return.
					 */
					$tmpstring .= ' ' . trim(preg_replace('/"/', ' ', $pieces[$k]));
...
function zen_create_sort_heading($sortby, $colnum, $heading)
{
	...
	if ($sortby)
	{
		$sort_prefix = '<a href="' . zen_href_link($_GET['main_page'], zen_get_all_get_params(array(
			'page',
			'info',
			'sort' 
		)) . 'page=1&sort=' . $colnum . ($sortby == $colnum . 'a' ? 'd' : 'a')) . '" title="' . zen_output_string(TEXT_SORT_PRODUCTS . ($sortby == $colnum . 'd' || substr($sortby, 0, 1) != $colnum ? TEXT_ASCENDINGLY : TEXT_DESCENDINGLY) . TEXT_BY . $heading) . '" class="productListing-heading">';
...
/*
 * function zen_get_uprid_OLD($prid, $params) { $uprid = $prid; if ( (is_array($params)) && (!strstr($prid, '{')) ) { while (list($option, $value) = each($params)) { $uprid = $uprid . '{' . $option . '}' . $value; } } return $uprid; }
 */
...
function zen_back_link($link_only = false)
{
	if (sizeof($_SESSION['navigation']->path) - 2 >= 0)
	{
		...
		$link = zen_href_link($_SESSION['navigation']->path[$back]['page'], zen_array_to_string($_SESSION['navigation']->path[$back]['get'], array(
			'action' 
		)), $_SESSION['navigation']->path[$back]['mode']);
	}
	else
	{
...
{
	if (!is_array($extraTags)) $extraTags = array(
		$extraTags 
	);
