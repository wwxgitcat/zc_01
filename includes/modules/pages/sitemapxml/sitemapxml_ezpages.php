...
if ($zen_SiteMapXML->SitemapOpen('ezpages', $last_date))
{
	...
	while(!$page_query->EOF)
	{
		...
		if ($page_query->fields['alt_url'] != '')
		{ // internal link
			if ($langParm != '')
			{
				$langParm = (strpos($page_query->fields['alt_url'], '?') === false ? '?' . ltrim('&', $langParm) : $langParm);
			}
			$link = (substr($page_query->fields['alt_url'], 0, 4) == 'http') ? $page_query->fields['alt_url'] : zen_href_link($page_query->fields['alt_url'] . $langParm, '', ($page_query->fields['page_is_ssl'] == '0' ? 'NONSSL' : 'SSL'), false, true, true);
			$parse_url = parse_url(str_replace('&', '&amp;', $link));
		...
		{ // use EZPage ID to create link
			$link = str_replace('&', '&amp;', zen_href_link(FILENAME_EZPAGES, 'id=' . $page_query->fields['pages_id'] . ((int)$page_query->fields['toc_chapter'] != 0 ? '&chapter=' . $page_query->fields['toc_chapter'] : '') . $langParm, ($page_query->fields['page_is_ssl'] == '0' ? 'NONSSL' : 'SSL'), false));
		}
		if ($link != false)
		{
