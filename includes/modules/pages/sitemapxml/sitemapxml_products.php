...
if ($zen_SiteMapXML->SitemapOpen('products', $last_date->fields['last_date']))
{
	$products = $db->Execute("SELECT p.products_id, GREATEST(p.products_date_added, IFNULL(p.products_last_modified, '0001-01-01 00:00:00')) AS last_date, p.products_sort_order AS priority, pd.language_id
                            ...
                            WHERE p.products_status = '1'" . $where . (SITEMAPXML_PRODUCTS_ORDERBY != '' ? "ORDER BY " . SITEMAPXML_PRODUCTS_ORDERBY : ''));
	...
	while(!$products->EOF)
	{
		...
		$link = str_replace('&', '&amp;', zen_href_link(zen_get_info_page($products->fields['products_id']), 'products_id=' . $products->fields['products_id'] . $langParm, 'NONSSL', false));
