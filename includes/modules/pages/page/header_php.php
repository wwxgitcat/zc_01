 ...
 */
/*
 * This "page" page is the display component of the ez-pages module It is called "page" instead of "ez-pages" due to the way the URL would display in the browser Aesthetically speaking, "page" is more professional in appearance than "ez-page" in the URL The EZ-Pages concept was adapted from the InfoPages contribution for Zen Cart v1.2.x, with thanks to Sunrom et al.
 */
...
/*
 * //prev/next for horiz now reset ($horiz_links); $counter = 0; while (list($key, $value) = each ($horiz_links)) { if ($value == $ezpage_id) { $position_h = $counter; $previous_hssl = '0'; if ($key == 0) { $previous_h = -1; // it was the first to be found } else { $previous_h = $horiz_links[$key - 1]; } $next_hssl = '0'; if ($horiz_links[$key + 1]) { $next_item_h = $horiz_links[$key + 1]; } else { $next_item_h = $horiz_links[0]; } } $last_h = $value; $counter++; } if ($previous_h == -1) $previous_h = $last_h;
 */
