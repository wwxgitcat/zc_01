...
$zco_notifier->notify('NOTIFY_CHECK_DOWNLOAD_HANDLER', array(
	$downloads 
));
...
/**
 * set notifier point .
 * .. we are ready to begin the actual download. An observer class could hook this notifier point and do something completely different, such as stamping PDFs etc.
 */
$zco_notifier->notify('NOTIFY_DOWNLOAD_READY_TO_START', array(
	$origin_filename,
	$browser_filename,
	$downloadFilesize,
 ...
 * However, some browsers and some software already installed on the PC may cause this to be overridden simply based on the filename.
 * In these cases, the user will have to manually choose "Save As" themselves.
...
if (DOWNLOAD_BY_REDIRECT == 'true')
{
	...
	$download_link = str_replace(array(
		'/',
		'\\' 
	), '_', $browser_filename);
