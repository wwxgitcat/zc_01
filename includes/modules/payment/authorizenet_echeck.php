...
class authorizenet_echeck extends base
{
	...
	function selection()
	{
		...
		$selection = array(
			...
			'fields' => array(
				array(
					'title' => MODULE_PAYMENT_AUTHORIZENET_ECHECK_TEXT_BANK_ROUTING_CODE,
				...
				array(
					'title' => MODULE_PAYMENT_AUTHORIZENET_ECHECK_TEXT_BANK_ACCOUNT_NUM,
				...
				array(
					'title' => MODULE_PAYMENT_AUTHORIZENET_ECHECK_TEXT_BANK_NAME,
				...
				array(
					'title' => MODULE_PAYMENT_AUTHORIZENET_ECHECK_TEXT_BANK_ACCOUNT_TYPE,
				...
				array(
					'title' => MODULE_PAYMENT_AUTHORIZENET_ECHECK_TEXT_BANK_ACCOUNTHOLDER,
		...
		if (MODULE_PAYMENT_AUTHORIZENET_ECHECK_WFSS_ENABLED == 'True')
		{
			...
			{
				$dl_states[] = array(
					'id' => $result->fields['zone_code'],
					'text' => $result->fields['zone_name'] 
				);
				$result->MoveNext();
			}
			$selection['fields'] = array_merge($selection['fields'], array(
				array(
					'title' => MODULE_PAYMENT_AUTHORIZENET_ECHECK_TEXT_CUST_TYPE,
				...
				array(
					'title' => MODULE_PAYMENT_AUTHORIZENET_ECHECK_TEXT_CUST_TAX_ID,
				...
				array(
					'title' => MODULE_PAYMENT_AUTHORIZENET_ECHECK_TEXT_DL_NUMBER,
				...
				array(
					'title' => MODULE_PAYMENT_AUTHORIZENET_ECHECK_TEXT_DL_STATE,
				...
				array(
					'title' => MODULE_PAYMENT_AUTHORIZENET_ECHECK_TEXT_DL_DOB_TEXT,
	...
	function before_process()
	{
		...
		if (MODULE_PAYMENT_AUTHORIZENET_ECHECK_WFSS_ENABLED == 'True')
		{
			...
			{
				$submit_data = array_merge($submit_data, array(
					'x_drivers_license_num' => zen_db_prepare_input($_POST['echeck_dl_num']),
	 ...
	 * Updates the order-status history data with the auth code from the transaction.
	 *
	 * @return boolean
	 */
	...
	{
		$error = array(
			'title' => MODULE_PAYMENT_AUTHORIZENET_ECHECK_TEXT_ERROR,
	...
	{
		return array(
			'MODULE_PAYMENT_AUTHORIZENET_ECHECK_STATUS',
			'MODULE_PAYMENT_AUTHORIZENET_ECHECK_LOGIN',
			'MODULE_PAYMENT_AUTHORIZENET_ECHECK_TXNKEY',
			'MODULE_PAYMENT_AUTHORIZENET_ECHECK_MD5HASH',
			'MODULE_PAYMENT_AUTHORIZENET_ECHECK_TESTMODE',
			'MODULE_PAYMENT_AUTHORIZENET_ECHECK_AUTHORIZATION_TYPE',
			'MODULE_PAYMENT_AUTHORIZENET_ECHECK_STORE_DATA',
			'MODULE_PAYMENT_AUTHORIZENET_ECHECK_EMAIL_CUSTOMER',
			...
			'MODULE_PAYMENT_AUTHORIZENET_ECHECK_WFSS_ENABLED',
			'MODULE_PAYMENT_AUTHORIZENET_ECHECK_DEBUGGING' 
		); // 'MODULE_PAYMENT_AUTHORIZENET_ECHECK_METHOD'
	}
	/**
	...
	function _debugActions($response, $order_time = '', $sessID = '')
	{
		...
		if (strstr(MODULE_PAYMENT_AUTHORIZENET_ECHECK_DEBUGGING, 'Log') || strstr(MODULE_PAYMENT_AUTHORIZENET_ECHECK_DEBUGGING, 'All') || (defined('AUTHORIZENET_DEVELOPER_MODE') && in_array(AUTHORIZENET_DEVELOPER_MODE, array(
			'on',
			'certify' 
		))))
		{
		...
		{
			zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, 'Authorizenet-eCheck Alert ' . $response[7] . ' ' . date('M-d-Y h:i:s') . ' ' . $response[6], $errorMessage, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, array(
	...
	function _doCapt($oID, $amt = 0, $currency = 'USD')
	{
		...
		if (isset($_POST['btndocapture']) && $_POST['btndocapture'] == MODULE_PAYMENT_AUTHORIZENET_ECHECK_ENTRY_CAPTURE_BUTTON_TEXT)
		{
			...
			/*
			 * if ($captureAmt == 0) { $messageStack->add_session(MODULE_PAYMENT_AUTHORIZENET_ECHECK_TEXT_INVALID_CAPTURE_AMOUNT, 'error'); $proceedToCapture = false; }
			 */
		}
