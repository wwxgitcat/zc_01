...
class paypaldp extends base
{
	...
	function paypaldp()
	{
		...
		if (IS_ADMIN_FLAG === true)
		{
			...
			if ($this->enabled)
			{
				if (((MODULE_PAYMENT_PAYPALDP_MERCHANT_COUNTRY == 'US' || MODULE_PAYMENT_PAYPALDP_MERCHANT_COUNTRY == 'Canada') && (MODULE_PAYMENT_PAYPALWPP_APISIGNATURE == '' || MODULE_PAYMENT_PAYPALWPP_APIUSERNAME == '' || MODULE_PAYMENT_PAYPALWPP_APIPASSWORD == '')) || (!defined('MODULE_PAYMENT_PAYPALWPP_STATUS') || MODULE_PAYMENT_PAYPALWPP_STATUS != 'True')) $this->title .= '<span class="alert"><strong> NOT CONFIGURED YET</strong></span>';
		...
		{
			if (CC_ENABLED_VISA == '1') $this->cards[] = array(
				'id' => 'Visa',
				'text' => 'Visa' 
			);
			if (CC_ENABLED_MC == '1') $this->cards[] = array(
				'id' => 'MasterCard',
				'text' => 'MasterCard' 
			);
			if (CC_ENABLED_MAESTRO == '1') $this->cards[] = array(
				'id' => 'Maestro',
				'text' => 'Maestro' 
			);
			if (CC_ENABLED_SWITCH == '1') $this->cards[] = array(
				'id' => 'Switch',
				'text' => 'Switch' 
			);
			if (CC_ENABLED_SOLO == '1') $this->cards[] = array(
				'id' => 'Solo',
				'text' => 'Solo' 
			);
		}
	...
	{
		global $order;
		$this->cc_type_check = 'var value = document.checkout_payment.paypalwpp_cc_type.value;' . 'if (value == "Solo" || value == "Maestro" || value == "Switch") {' . '    document.checkout_payment.paypalwpp_cc_issue_month.disabled = false;' . '    document.checkout_payment.paypalwpp_cc_issue_year.disabled = false;' . '    document.checkout_payment.paypalwpp_cc_checkcode.disabled = false;' . '    if (document.checkout_payment.paypalwpp_cc_issuenumber) document.checkout_payment.paypalwpp_cc_issuenumber.disabled = false;' . '} else {' . '    if (document.checkout_payment.paypalwpp_cc_issuenumber) document.checkout_payment.paypalwpp_cc_issuenumber.disabled = true;' . '    if (document.checkout_payment.paypalwpp_cc_issue_month) document.checkout_payment.paypalwpp_cc_issue_month.disabled = true;' . '    if (document.checkout_payment.paypalwpp_cc_issue_year) document.checkout_payment.paypalwpp_cc_issue_year.disabled = true;' . '    document.checkout_payment.paypalwpp_cc_checkcode.disabled = false;' . '}';
		...
		$fieldsArray = array();
		$fieldsArray[] = array(
			'title' => MODULE_PAYMENT_PAYPALDP_TEXT_CREDIT_CARD_FIRSTNAME,
			'field' => zen_draw_input_field('paypalwpp_cc_firstname', $order->billing['firstname'], 'id="' . $this->code . '-cc-ownerf"' . $onFocus . ' autocomplete="off"') . '<script type="text/javascript">function paypalwpp_cc_type_check() { ' . $this->cc_type_check . ' } </script>',
			'tag' => $this->code . '-cc-ownerf' 
		);
		$fieldsArray[] = array(
			'title' => MODULE_PAYMENT_PAYPALDP_TEXT_CREDIT_CARD_LASTNAME,
		...
		if (sizeof($this->cards) > 0) $fieldsArray[] = array(
			'title' => MODULE_PAYMENT_PAYPALDP_TEXT_CREDIT_CARD_TYPE,
		...
		$fieldsArray[] = array(
			'title' => MODULE_PAYMENT_PAYPALDP_TEXT_CREDIT_CARD_NUMBER,
			'field' => zen_draw_input_field('paypalwpp_cc_number', $ccnum, 'id="' . $this->code . '-cc-number"' . $onFocus . ' autocomplete="off"'),
			'tag' => $this->code . '-cc-number' 
		);
		$fieldsArray[] = array(
			'title' => MODULE_PAYMENT_PAYPALDP_TEXT_CREDIT_CARD_EXPIRES,
		...
		$fieldsArray[] = array(
			'title' => MODULE_PAYMENT_PAYPALDP_TEXT_CREDIT_CARD_CHECKNUMBER,
		...
		if (MODULE_PAYMENT_PAYPALDP_MERCHANT_COUNTRY == 'UK' && (CC_ENABLED_MAESTRO == '1' || CC_ENABLED_SWITCH == '1' || CC_ENABLED_SOLO == '1'))
		{
			...
			array_splice($selection['fields'], 4, 0, array(
				array(
					'title' => MODULE_PAYMENT_PAYPALDP_TEXT_CREDIT_CARD_ISSUE,
			...
			array_splice($selection['fields'], 4, 0, array(
				array(
					'title' => MODULE_PAYMENT_PAYPALDP_TEXT_CREDIT_CARD_MAESTRO_ISSUENUMBER,
			...
			$selection['fields'][] = array(
				'title' => '',
				'field' => '<div id="' . $this->code . '-cc-securetext"><p>' . '<a href="javascript:void window.open(\'vbv_learn_more.html\',\'vbv_service\',\'width=550,height=450\')">' . zen_image(DIR_WS_IMAGES . '3ds/vbv_learn_more.gif') . '</a>' . '<a href="javascript:void window.open(\'mcs_learn_more.html\',\'mcsc_service\',\'width=550,height=450\')">' . zen_image(DIR_WS_IMAGES . '3ds/mcsc_learn_more.gif') . '</a>' . '</p>' . '<p>' . TEXT_3DS_CARD_MAY_BE_ENROLLED . '</p></div>',
	...
	function pre_confirmation_check()
	{
		...
		$result = $cc_validation->validate($_POST['paypalwpp_cc_number'], $_POST['paypalwpp_cc_expires_month'], $_POST['paypalwpp_cc_expires_year'], (isset($_POST['paypalwpp_cc_issue_month']) ? $_POST['paypalwpp_cc_issue_month'] : ''), (isset($_POST['paypalwpp_cc_issue_year']) ? $_POST['paypalwpp_cc_issue_year'] : ''));
		$error = '';
		switch($result)
		{
			case 1:
		...
		if (MODULE_PAYMENT_PAYPALDP_MERCHANT_COUNTRY == 'UK' && (!isset($_POST['MD'])))
		{
			...
			{
				$_SESSION['3Dsecure_merchantData'] = serialize(array(
					'im' => $_POST['paypalwpp_cc_issue_month'],
					'iy' => $_POST['paypalwpp_cc_issue_year'],
					'in' => $_POST['paypalwpp_cc_issuenumber'],
					'fn' => $_POST['paypalwpp_cc_firstname'],
					'ln' => $_POST['paypalwpp_cc_lastname'] 
				));
				global $order_total_modules;
				$calculatedOrderTotal = $order_total_modules->pre_confirmation_check(TRUE);
				$lookup_data_array = array(
				...
				else
				{
					...
					if ($shouldContinue != 'Y')
					{
						...
						zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, MODULE_PAYMENT_PAYPALDP_TEXT_EMAIL_ERROR_SUBJECT . ' ' . $reason, $errorText, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, array(
							'EMAIL_MESSAGE_HTML' => nl2br($errorText) 
						), 'paymentalert');
						zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL', true, false));
					}
					else
		...
		$confirmation = array(
			...
			'fields' => array(
				...
				array(
					...
					(isset($_POST['paypalwpp_cc_issuenumber']) ? array(
						'title' => MODULE_PAYMENT_PAYPALDP_TEXT_ISSUE_NUMBER,
	...
	function before_process()
	{
		...
		if (!in_array($cc_validation->cc_type, array(
			'Visa',
			'MasterCard',
			'Switch',
			'Solo',
		...
		if (MODULE_PAYMENT_PAYPALDP_MERCHANT_COUNTRY == 'UK')
		{
			...
			{ // authentication attempt required?
				...
				if (isset($_SESSION['3Dsecure_enroll_lookup_attempted']) == false || strcasecmp($_SESSION['3Dsecure_enroll_lookup_attempted'], 'Y') != 0)
				{
					...
					$errorText = $reason . "\n\nProblem occurred while customer " . $_SESSION['customer_id'] . ' ' . $_SESSION['customer_first_name'] . ' ' . $_SESSION['customer_last_name'] . ' was attempting checkout with 3D-Secure authentication.';
					$errorText .= $this->code;
					zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, MODULE_PAYMENT_PAYPALDP_TEXT_EMAIL_ERROR_SUBJECT, $errorText, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, array(
						'EMAIL_MESSAGE_HTML' => nl2br($errorText) 
					), 'paymentalert');
					zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL', true, false));
				}
		...
		$doPayPal = $this->paypal_init();
		$optionsAll = array_merge($options, array(
		...
		{
			$optionsShip = array(
				'SHIPTONAME' => ($order->delivery['name'] == '' ? $order->delivery['firstname'] . ' ' . $order->delivery['lastname'] : $order->delivery['name']),
	...
	function after_process()
	{
		...
		if ($this->fmfResponse != '')
		{
			...
			zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, MODULES_PAYMENT_PAYPALDP_TEXT_EMAIL_FMF_SUBJECT . ' (' . $insert_id . ')', $detailedMessage, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, array(
				'EMAIL_MESSAGE_HTML' => nl2br($detailedMessage) 
			), 'paymentalert');
		}
		
		// add a new OSH record for this order's PP details
		$commentString = "Transaction ID: :transID: " . (isset($this->responsedata['PPREF']) ? "\nPPRef: " . $this->responsedata['PPREF'] : "") . (isset($this->responsedata['AUTHCODE']) ? "\nAuthCode: " . $this->responsedata['AUTHCODE'] : "") . "\nPayment Type: :pmtType: " . ($this->payment_time != '' ? "\nTimestamp: :pmtTime: " : "") . "\nPayment Status: :pmtStatus: " . (isset($this->responsedata['auth_exp']) ? "\nAuth-Exp: " . $this->responsedata['auth_exp'] : "") . ($this->avs != 'N/A' ? "\nAVS Code: " . $this->avs . "\nCVV2 Code: " . $this->cvv2 : '') . (trim($this->amt) != '' ? "\nAmount: :orderAmt: " : "");
	...
	/**
	 * Used to read details of existing transactions.
	 * FOR FUTURE USE.
	 */
	 ...
	 * Returns true if the status key is found.
	 */
	function check()
	{
		global $db;
	...
	{
		$keys_list = array(
			'MODULE_PAYMENT_PAYPALDP_STATUS',
			'MODULE_PAYMENT_PAYPALDP_SORT_ORDER',
			'MODULE_PAYMENT_PAYPALDP_ZONE',
			'MODULE_PAYMENT_PAYPALDP_ORDER_STATUS_ID',
			'MODULE_PAYMENT_PAYPALDP_ORDER_PENDING_STATUS_ID',
			'MODULE_PAYMENT_PAYPALDP_REFUNDED_STATUS_ID',
			'MODULE_PAYMENT_PAYPALDP_TRANSACTION_MODE',
			'MODULE_PAYMENT_PAYPALDP_CURRENCY',
			'MODULE_PAYMENT_PAYPALDP_MERCHANT_COUNTRY',
			'MODULE_PAYMENT_PAYPALDP_EC_RETURN_FMF_DETAILS',
			'MODULE_PAYMENT_PAYPALDP_SERVER',
			'MODULE_PAYMENT_PAYPALDP_DEBUGGING' 
		);
		if (MODULE_PAYMENT_PAYPALDP_MERCHANT_COUNTRY == 'UK')
		{
			$keys_list = array_merge($keys_list, array(
				'MODULE_PAYMENT_PAYPALDP_CARDINAL_PROCESSOR',
				'MODULE_PAYMENT_PAYPALDP_CARDINAL_MERCHANT',
				'MODULE_PAYMENT_PAYPALDP_CARDINAL_PASSWORD',
				'MODULE_PAYMENT_PAYPALDP_CARDINAL_AUTHENTICATE_REQ' 
			));
		}
		return $keys_list;
	}
	...
	{
		if ((defined('MODULE_PAYMENT_PAYPALDP_STATUS') && MODULE_PAYMENT_PAYPALDP_STATUS == 'True') && !empty($_SESSION['paypal_ec_token']) && !empty($_SESSION['paypal_ec_payer_id']) && !empty($_SESSION['paypal_ec_payer_info']))
		{
			return true;
		}
	}
	...
	function _doDebug($subject = 'PayPal debug data', $data, $useSession = true)
	{
		if (MODULE_PAYMENT_PAYPALDP_DEBUGGING == 'Log and Email')
		{
			...
			zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, $subject, $this->code . "\n" . $data, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, array(
				'EMAIL_MESSAGE_HTML' => nl2br($this->code . "\n" . $data) 
			), 'debug');
		}
	}
	/**
	...
	function paypal_init()
	{
		...
		{
			$doPayPal = new paypal_curl(array(
				'mode' => 'payflow',
			...
			$doPayPal->_endpoints = array(
				'live' => 'https://payflowpro.paypal.com/transaction',
	...
	function getLineItemDetails($restrictedCurrency)
	{
		...
		if (sizeof($order_totals))
		{
			...
			for($i = 0, $n = sizeof($order_totals); $i < $n; $i++)
			{
				...
				if (in_array($order_totals[$i]['code'], array(
					'ot_total',
					'ot_subtotal',
					'ot_tax',
					'ot_shipping' 
				)) || strstr($order_totals[$i]['code'], 'insurance'))
				{
		...
		{
			$optionsLI["L_NAME$k"] = str_replace(array(
				'&',
				'=',
				'%' 
			), '*', $optionsLI["L_NAME$k"]);
			...
			if (isset($optionsLI["L_NUMBER$k"]))
			{
				...
				{
					$optionsLI["L_NUMBER$k"] = str_replace(array(
						'&',
						'=',
						'%' 
		...
		if (!defined('PAYPALWPP_SKIP_LINE_ITEM_DETAIL_FORMATTING') || PAYPALWPP_SKIP_LINE_ITEM_DETAIL_FORMATTING != 'true' || in_array($order->info['currency'], array(
			'JPY',
			'NOK',
			'HUF' 
		)))
		...
		$this->zcLog('getLineItemDetails 8', 'checking subtotals... ' . "\n" . print_r(array_merge(array(
			'calculated total' => number_format($stAll, ((int)$currencies->get_decimal_places($restrictedCurrency) == 0 ? 0 : 2)) 
	...
	function _errorHandler($response, $operation = '', $ignore_codes = '')
	{
		...
		if (in_array($operation, array(
			...
			'DoDirectPayment' 
		)) && $response['PAYMENTSTATUS'] == 'Pending' && $response['L_ERRORCODE0'] == 11610)
		{
			...
			if ($response['ACK'] == 'SuccessWithWarning' && isset($response['L_FMFPENDINGID0']))
			{
				...
				{
					$this->fmfErrors[] = array(
						'key' => $response['L_FMFPENDINGID' . $i],
						'status' => $response['L_FMFPENDINGID' . $i],
						'desc' => $response['L_FMFPENDINGDESCRIPTION' . $i] 
					);
				}
			}
		...
		switch($operation)
		{
			case 'DoDirectPayment':
				if ($basicError || ((isset($_SESSION['paypal_ec_token']) && isset($response['TOKEN'])) && $_SESSION['paypal_ec_token'] != urldecode($response['TOKEN'])))
				{
					...
					zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, MODULE_PAYMENT_PAYPALDP_TEXT_EMAIL_ERROR_SUBJECT . ' (' . zen_uncomment($errorNum) . ')', zen_uncomment($detailedEmailMessage), STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, array(
						'EMAIL_MESSAGE_HTML' => nl2br(zen_uncomment($detailedEmailMessage)) 
					), 'paymentalert');
			...
			default:
				if ($basicError)
				{
					...
					if ($detailedEmailMessage != '') zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, MODULE_PAYMENT_PAYPALDP_TEXT_EMAIL_ERROR_SUBJECT . ' (' . zen_uncomment($errorNum) . ')', zen_uncomment($detailedMessage . $explain), STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, array(
						'EMAIL_MESSAGE_HTML' => nl2br(zen_uncomment($detailedEmailMessage)) 
					), 'paymentalert');
	...
	function get3DSecureLookupResponse($lookup_data_array)
	{
		...
		$data .= '</CardinalMPI>';
		$debugData = str_replace(array(
			'[CDATA[' . $this->escapeXML(MODULE_PAYMENT_PAYPALDP_CARDINAL_MERCHANT) . ']]',
			'[CDATA[' . $this->escapeXML(MODULE_PAYMENT_PAYPALDP_CARDINAL_PASSWORD) . ']]',
			$this->escapeXML($lookup_data_array['cc3d_card_number']),
		...
		if ($errorNo != 0)
		{
			...
			zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, substr($errorDesc, 0, 75) . ' (' . $errorNo . ')', $errorText, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, array(
				'EMAIL_MESSAGE_HTML' => nl2br($errorText) 
			), 'paymentalert');
		}
		...
		else if ($errorNo == 1001)
		{ // merchant has an account configuration problem to fix
			$errorText = CENTINEL_ERROR_CODE_1001 . ' - ' . CENTINEL_ERROR_CODE_1001_DESC;
			$errorText .= "\n\nProblem occurred while customer " . $_SESSION['customer_id'] . ' ' . $_SESSION['customer_first_name'] . ' ' . $_SESSION['customer_last_name'] . ' was attempting checkout with 3D-Secure authentication.';
			zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, CENTINEL_ERROR_CODE_1001_DESC . ' (' . CENTINEL_ERROR_CODE_1001 . ')', $errorText, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, array(
				'EMAIL_MESSAGE_HTML' => nl2br($errorText) 
			), 'paymentalert');
			$continue_flag = 'Y';
		}
	...
	function send3DSecureHttp($url, $data, $debugData)
	{
		...
		if ((strpos($url, "http://") === 0) || (strpos($url, "https://") === 0))
		{
			...
			if (!$succeeded)
			{
				...
				$errorText .= "\n\nProblem occurred while customer " . $_SESSION['customer_id'] . ' ' . $_SESSION['customer_first_name'] . ' ' . $_SESSION['customer_last_name'] . ' was attempting checkout with 3D-Secure authentication.';
				if ($error != '-') $errorText .= "\n\nCURL error: " . $error;
				zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, CENTINEL_ERROR_CODE_8030_DESC . ' (' . CENTINEL_ERROR_CODE_8030 . ')', $errorText, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, array(
					'EMAIL_MESSAGE_HTML' => nl2br($errorText) 
				), 'paymentalert');
			}
			else if (strpos($result, "<CardinalMPI>") === false)
			{
				...
				$errorText .= "\n\nProblem occurred while customer " . $_SESSION['customer_id'] . ' ' . $_SESSION['customer_first_name'] . ' ' . $_SESSION['customer_last_name'] . ' was attempting checkout with 3D-Secure authentication.';
				zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, CENTINEL_ERROR_CODE_8010_DESC . ' (' . CENTINEL_ERROR_CODE_8010 . ')', $errorText, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, array(
					'EMAIL_MESSAGE_HTML' => nl2br($errorText) 
				), 'paymentalert');
			}
			else
			{
				...
				if (strpos($result, "<ErrorNo>4243") > 0)
				{
					...
					$errorText .= "\n\nProblem occurred while customer " . $_SESSION['customer_id'] . ' ' . $_SESSION['customer_first_name'] . ' ' . $_SESSION['customer_last_name'] . ' was attempting checkout with 3D-Secure authentication.';
					zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, CENTINEL_ERROR_CODE_4243_DESC . ' (' . CENTINEL_ERROR_CODE_4243 . ')', $errorText, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, array(
						'EMAIL_MESSAGE_HTML' => nl2br($errorText) 
					), 'paymentalert');
				}
			}
		...
		else
		{
			...
			$errorText .= "\n\nProblem occurred while customer " . $_SESSION['customer_id'] . ' ' . $_SESSION['customer_first_name'] . ' ' . $_SESSION['customer_last_name'] . ' was attempting checkout with 3D-Secure authentication.';
			zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, CENTINEL_ERROR_CODE_8000_DESC . ' (' . CENTINEL_ERROR_CODE_8000 . ')', $errorText, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, array(
				'EMAIL_MESSAGE_HTML' => nl2br($errorText) 
			), 'paymentalert');
		}
	...
	function requiresLookup($info)
	{
		...
		if (in_array(strtoupper($cardType), array(
			'VISA',
			'MASTERCARD',
			'JCB',
			'MAESTRO' 
