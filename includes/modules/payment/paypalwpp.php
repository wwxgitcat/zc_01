...
class paypalwpp extends base
{
	...
	function before_process()
	{
		...
		if ($_SESSION['paypal_ec_payer_info']['ship_street_1'] != '' && strtoupper($_SESSION['paypal_ec_payer_info']['ship_address_status']) != 'NONE')
		{
			$options = array_merge($options, array(
				'SHIPTONAME' => $_SESSION['paypal_ec_payer_info']['ship_name'],
		...
		$response = $doPayPal->DoExpressCheckoutPayment($_SESSION['paypal_ec_token'], $_SESSION['paypal_ec_payer_id'], $options);
	...
	function keys()
	{
		...
		$keys_list = array(
			'MODULE_PAYMENT_PAYPALWPP_STATUS',
			'MODULE_PAYMENT_PAYPALWPP_SORT_ORDER',
			'MODULE_PAYMENT_PAYPALWPP_ZONE',
			'MODULE_PAYMENT_PAYPALWPP_ECS_BUTTON',
			'MODULE_PAYMENT_PAYPALWPP_ORDER_STATUS_ID',
			'MODULE_PAYMENT_PAYPALWPP_ORDER_PENDING_STATUS_ID',
			'MODULE_PAYMENT_PAYPALWPP_REFUNDED_STATUS_ID',
			'MODULE_PAYMENT_PAYPALWPP_CONFIRMED_ADDRESS',
			'MODULE_PAYMENT_PAYPALWPP_AUTOSELECT_CHEAPEST_SHIPPING',
			'MODULE_PAYMENT_PAYPALWPP_SKIP_PAYMENT_PAGE',
			'MODULE_PAYMENT_PAYPALWPP_NEW_ACCT_NOTIFY',
			'MODULE_PAYMENT_PAYPALWPP_TRANSACTION_MODE',
			'MODULE_PAYMENT_PAYPALWPP_CURRENCY',
			'MODULE_PAYMENT_PAYPALWPP_BRANDNAME',
			'MODULE_PAYMENT_PAYPALEC_ALLOWEDPAYMENT',
			'MODULE_PAYMENT_PAYPALWPP_PAGE_STYLE',
			'MODULE_PAYMENT_PAYPALWPP_APIUSERNAME',
			'MODULE_PAYMENT_PAYPALWPP_APIPASSWORD',
			'MODULE_PAYMENT_PAYPALWPP_APISIGNATURE',
			'MODULE_PAYMENT_PAYPALWPP_MODULE_MODE',
			'MODULE_PAYMENT_PAYPALWPP_SERVER',
		...
		if (IS_ADMIN_FLAG === true && (PAYPAL_DEV_MODE == 'true' || strstr(MODULE_PAYMENT_PAYPALWPP_MODULE_MODE, 'Payflow')))
		{
			$keys_list = array_merge($keys_list, array(
				'MODULE_PAYMENT_PAYPALWPP_PFPARTNER',
				'MODULE_PAYMENT_PAYPALWPP_PFVENDOR',
				'MODULE_PAYMENT_PAYPALWPP_PFUSER',
	...
	{
		if ((defined('MODULE_PAYMENT_PAYPALWPP_STATUS') && MODULE_PAYMENT_PAYPALWPP_STATUS == 'True') && !empty($_SESSION['paypal_ec_token']) && !empty($_SESSION['paypal_ec_payer_id']) && !empty($_SESSION['paypal_ec_payer_info']))
		{
	...
	function paypal_init()
	{
		...
		$doPayPal->_trxtype = (in_array(MODULE_PAYMENT_PAYPALWPP_TRANSACTION_MODE, array(
			'Auth Only',
			'Order' 
		))) ? 'A' : 'S';
	...
	/**
	 * Used to authorize part of a given previously-initiated transaction.
	 * FOR FUTURE USE.
	 */
	...
	function getLanguageCode()
	{
		...
		else if (in_array(strtoupper($_SESSION['languages_code']), array(
			'EN',
			'US',
			'AU',
			'DE',
	...
	function ec_step1()
	{
		...
		$this->zcLog('ec_step1 - 2 -submit', print_r(array_merge($options, array(
	...
	/**
	 * This method is for step 2 of the express checkout option.
	 * This
	...
	function ec_step2()
	{
		...
		if ($_SESSION['cart']->get_content_type() != 'virtual') if ($response['SHIPTONAME'] . $response['SHIPTOSTREET'] . $response['SHIPTOSTREET2'] . $response['SHIPTOCITY'] . $response['SHIPTOSTATE'] . $response['SHIPTOZIP'] . $response['SHIPTOCOUNTRYCODE'] == '')
		{
		...
		if ($_SESSION['cart']->get_content_type('true') > 0 || in_array($_SESSION['cart']->content_type, array(
			'mixed',
			'virtual' 
		...
		$step2_shipto = array(
			'ship_name' => urldecode($response['SHIPTONAME']),
	 ...
	 * duplicate entries to the address book.
	 * On a successful match it
	...
	function findMatchingAddressBookEntry($customer_id, $address_question_arr)
	{
		...
		// debug
		$this->zcLog('findMatchingAddressBookEntry - 1-stats', 'lookups:' . "\n" . print_r(array_merge($country->fields, array(
			'zone_country_id' => $country_zone_check->fields['zone_id'] 
	...
	function addAddressBookEntry($customer_id, $address_question_arr, $make_default = false)
	{
		...
		$sql_data_array = array(
			...
			array(
				...
				'value' => $address_question_arr['street_address'],
				'type' => 'string' 
			),
			array(
				'fieldName' => 'entry_postcode',
				'value' => $address_question_arr['postcode'],
				'type' => 'string' 
			),
			array(
				'fieldName' => 'entry_city',
				'value' => $address_question_arr['city'],
				'type' => 'string' 
			),
			array(
		...
		if ($address_question_arr['company'] != '' && $address_question_arr['company'] != $address_question_arr['name']) array(
			'fieldName' => 'entry_company',
			'value' => $address_question_arr['company'],
			'type' => 'string' 
		);
		$sql_data_array[] = array(
			'fieldName' => 'entry_gender',
			'value' => $address_question_arr['payer_gender'],
			'type' => 'enum:m|f' 
		);
		$sql_data_array[] = array(
			'fieldName' => 'entry_suburb',
			'value' => $address_question_arr['suburb'],
			'type' => 'string' 
		);
		if ($zone_id > 0)
		{
		...
		else
		{
			...
			$sql_data_array[] = array(
				'fieldName' => 'entry_state',
				'value' => $address_question_arr['state'],
				'type' => 'string' 
			);
		}
		$sql_data_array[] = array(
		...
		switch($operation)
		{
			case 'SetExpressCheckout':
				if ($basicError)
				{
					...
					if ($detailedEmailMessage != '') zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, MODULE_PAYMENT_PAYPALWPP_TEXT_EMAIL_ERROR_SUBJECT . ' (' . zen_uncomment($errorNum) . ')', zen_uncomment($detailedEmailMessage), STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, array(
						'EMAIL_MESSAGE_HTML' => zen_uncomment($detailedMessage) 
					), 'paymentalert');
			...
			default:
				if ($basicError)
				{
					...
					if ($detailedEmailMessage != '') zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, MODULE_PAYMENT_PAYPALWPP_TEXT_EMAIL_ERROR_SUBJECT . ' (' . zen_uncomment($errorNum) . ')', zen_uncomment($detailedMessage), STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, array(
