...
if (method_exists($this, '_doRefund'))
{
	...
	$outputRefund .= zen_draw_form('pprefund', FILENAME_ORDERS, zen_get_all_get_params(array(
		'action' 
...
if (method_exists($this, '_doAuth') && !isset($response['RESPMSG']))
{
	...
	$outputAuth .= zen_draw_form('ppauth', FILENAME_ORDERS, zen_get_all_get_params(array(
		'action' 
	)) . 'action=doAuth', 'post', '', true);
...
if (method_exists($this, '_doCapt'))
{
	...
	$outputCapt .= zen_draw_form('ppcapture', FILENAME_ORDERS, zen_get_all_get_params(array(
		'action' 
	)) . 'action=doCapture', 'post', '', true) . zen_hide_session_id();
...
if (method_exists($this, '_doVoid'))
{
	...
	$outputVoid .= zen_draw_form('ppvoid', FILENAME_ORDERS, zen_get_all_get_params(array(
		'action' 
	)) . 'action=doVoid', 'post', '', true) . zen_hide_session_id();
...
if (isset($response['RESPMSG']) || defined('MODULE_PAYMENT_PAYFLOW_STATUS'))
{ // payflow
	$output .= $outputPFmain;
...
{ // PayPal
	...
	if (defined('MODULE_PAYMENT_PAYPALWPP_STATUS') || defined('MODULE_PAYMENT_PAYPALDP_STATUS'))
	{
		...
		$output .= $outputStartBlock;
		$output .= $outputStartBlock;
		if ($response['TRANSACTION_TYPE'] == 'Authorization' || (in_array($response['TRANSACTIONTYPE'], array(
			'cart',
			'expresscheckout',
			'webaccept' 
		)) && $response['PAYMENTTYPE'] == 'instant' && $response['PENDINGREASON'] == 'authorization') || (isset($_GET['authcapt']) && $_GET['authcapt'] == 'on'))
		{
