...
class paypal_curl extends base
{
	 ...
	 * *not* be done to avoid security problems.
	 * File names are
	...
	/**
	 * Options for cURL.
	 * Defaults to preferred (constant) options.
	 */
	var $_curlOptions = array(
		CURLOPT_HEADER => 0,
	...
	/**
	 * Sales or authorizations? For the U.K.
	 * this will always be 'S'
	...
	{
		$values = array_merge($options, array(
			'TOKEN' => $token,
			'PAYERID' => $payerId 
		));
	...
	function _request($values, $operation, $requestId = null)
	{
		...
		{
			return array(
				'RESULT' => 'PayPal credentials not set. Cannot proceed.' 
			);
			die('NOTCONFIGURED');
		}
		if ($this->checkHasApiCredentials() === FALSE && (!in_array($operation, array(
			'SetExpressCheckout',
			'GetExpressCheckoutDetails',
			'DoExpressCheckoutPayment' 
		))))
		{
			return array(
				'RESULT' => 'Unauthorized: Unilateral' 
			);
		}
		...
		if ((!in_array($operation, array(
			...
			'GetTransactionDetails',
			'TransactionSearch' 
		)) && !strstr($response, '&ACK=Success'))) $this->_logTransaction($operation, $this->_getElapsed($start), $response, $errors . ($commErrNo != 0 ? "\n" . print_r($commInfo, true) : ''));
	 ...
	 * associative array.
	 * Doesn't handle length tags in the response
	...
	function _logTransaction($operation, $elapsed, $response, $errors)
	{
		...
		if ($this->_logLevel > 0 || $success == FALSE)
		{
			...
			{
				zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, 'PayPal Debug log - ' . $operation, $message, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, array(
	...
	function _sanitizeLog($log, $allsensitive = false)
	{
		if (is_array($log))
		{
			foreach(array_keys($log) as $key)
			{
				...
				if ($allsensitive && in_array($key, array(
					'BUTTONSOURCE',
					'VERSION',
					'SIGNATURE',
					'USER',
	 ...
	 *        
	 * @param integer $start
	 *        	Start time including microseconds.
	 *        	
