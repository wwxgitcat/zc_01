...
function ipn_debug_email($message, $email_address = '', $always_send = false, $subjecttext = 'IPN DEBUG message')
{
	...
	if ((defined('MODULE_PAYMENT_PAYPALWPP_DEBUGGING') && MODULE_PAYMENT_PAYPALWPP_DEBUGGING == 'Log and Email') || (defined('MODULE_PAYMENT_PAYPAL_IPN_DEBUG') && MODULE_PAYMENT_PAYPAL_IPN_DEBUG == 'Log and Email') || $always_send)
	{
		...
		zen_mail(STORE_OWNER, $email_address, $subjecttext . ' (' . $paypal_instance_id . ') #' . $paypal_error_counter, $message, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, array(
...
function ipn_create_order_update_array($txn_type)
{
	...
	if (isset($_POST['address_street']) && $_POST['address_street'] != '') $sql_data_array = array_merge($sql_data_array, array(
		'address_name' => $_POST['address_name'],
		'address_street' => $_POST['address_street'],
		'address_city' => $_POST['address_city'],
		'address_state' => $_POST['address_state'],
...
function ipn_getLineItemDetails($restrictedCurrency)
{
	...
	{
		$optionsLI["item_name_$k"] = str_replace(array(
			'&',
			'=',
			'%' 
		), '*', $optionsLI["item_name_$k"]);
		...
		if (isset($optionsLI["item_number_$k"]))
		{
			...
			{
				$optionsLI["item_number_$k"] = str_replace(array(
					'&',
					'=',
					'%' 
				), '*', $optionsLI["item_number_$k"]);
...
{
	if (defined('IPN_EXTRA_DEBUG_DETAILS') && IPN_EXTRA_DEBUG_DETAILS != '') ipn_add_error_log($stage . ($message != '' ? ': ' . $message : ''));
}
