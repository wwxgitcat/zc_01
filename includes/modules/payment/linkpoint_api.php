...
class linkpoint_api
{
	...
	function selection()
	{
		...
		$selection = array(
			...
			'fields' => array(
				array(
					'title' => MODULE_PAYMENT_LINKPOINT_API_TEXT_CREDIT_CARD_OWNER,
				...
				array(
					'title' => MODULE_PAYMENT_LINKPOINT_API_TEXT_CREDIT_CARD_NUMBER,
					'field' => zen_draw_input_field('linkpoint_api_cc_number', $ccnum, 'id="' . $this->code . '-cc-number"' . $onFocus . ' autocomplete="off"'),
					'tag' => $this->code . '-cc-number' 
				),
				array(
					'title' => MODULE_PAYMENT_LINKPOINT_API_TEXT_CREDIT_CARD_EXPIRES,
				...
				array(
					'title' => MODULE_PAYMENT_LINKPOINT_API_TEXT_CVV,
		...
		if (($result == false) || ($result < 1))
		{
			...
			if (MODULE_PAYMENT_LINKPOINT_API_STORE_DATA == 'True')
			{
				...
				$sql_data_array = array(
					...
					array(
						'fieldName' => 'transaction_response_time',
						'value' => 'N/A',
						'type' => 'string' 
					),
					array(
						'fieldName' => 'r_error',
						'value' => '**CC Info Failed Validation during pre-processing**',
						'type' => 'string' 
					),
					array(
					...
					array(
						'fieldName' => 'message',
						'value' => $message . ' -- ' . $all_response_info,
						'type' => 'string' 
					),
					array(
					...
					array(
						'fieldName' => 'chargetotal',
						'value' => 0,
						'type' => 'string' 
					),
					array(
						'fieldName' => 'cc_expire',
						'value' => $cc_month . '/' . $cc_year,
						'type' => 'string' 
					),
					array(
						'fieldName' => 'ordertype',
	...
	function before_process()
	{
		...
		if ($surcharges == 0 && $creditsApplied == 0 && $order->info['total'] >= $order->info['subtotal'] && sizeof($order->products) <= 20)
		{
			...
			/*
			 * // deal with surcharges/fees $num_line_items++; $myorder["items"][$num_line_items]['id'] = 'Surcharge'; $myorder["items"][$num_line_items]['description'] = $order_totals[$i]['title']; $myorder["items"][$num_line_items]['quantity'] = 1; $myorder["items"][$num_line_items]['price'] = number_format($order_totals[$i]['value'], 2, '.', ''); $myorder["subtotal"] += $surcharges;
			 */
		...
		{
			if (in_array(substr($result['r_error'], 0, 10), array(
				'SGS-002301',
				'SGS-010503',
				'SGS-005003' 
			)))
		...
		$sql_data_array = array(
			...
			array(
				...
				'value' => $result['r_time'],
				'type' => 'string' 
			), // The time+date of the transaction server response.
			array(
				'fieldName' => 'r_error',
				'value' => $result['r_error'],
				'type' => 'string' 
			),
			array(
				'fieldName' => 'customer_id',
			...
			array(
				'fieldName' => 'transaction_result',
				'value' => $result['r_approved'],
				'type' => 'string' 
			), // Transaction result: APPROVED, DECLINED, or FRAUD.
			array(
				'fieldName' => 'message',
				'value' => $result['r_message'] . "\n" . $all_response_info,
				'type' => 'string' 
			), // Any message returned by the processor; e.g., CALL VOICE CENTER.
			array(
				'fieldName' => 'transaction_time',
				'value' => $result['r_tdate'],
				'type' => 'string' 
			), // A server time-date stamp for this transaction.
			array(
				'fieldName' => 'transaction_reference_number',
				'value' => $result['r_ref'],
				'type' => 'string' 
			), // Reference number returned by the CC processor.
			array(
				'fieldName' => 'fraud_score',
				'value' => $result['r_score'],
				'type' => 'integer' 
			), // LinkShield fraud risk score.
			array(
				'fieldName' => 'cc_number',
				'value' => $cc_number,
				'type' => 'string' 
			...
			array(
				'fieldName' => 'ordertype',
				'value' => $myorder['ordertype'],
				'type' => 'string' 
			), // transaction type: PREAUTH or SALE
			array(
				'fieldName' => 'date_added',
				'value' => 'now()',
				'type' => 'noquotestring' 
	...
	function after_process()
	{
		...
		switch(MODULE_PAYMENT_LINKPOINT_API_TRANSACTION_MODE_RESPONSE)
		{
			case "LIVE: Production":
				...
				break;
			case "TESTING: Successful":
				$comments .= ' ' . ALERT_LINKPOINT_API_TEST_FORCED_SUCCESSFUL;
				break;
			case "TESTING: Decline":
				$comments .= ' ' . ALERT_LINKPOINT_API_TEST_FORCED_DECLINED;
				break;
		}
	...
	{
		$error = array(
			'title' => MODULE_PAYMENT_LINKPOINT_API_TEXT_ERROR,
