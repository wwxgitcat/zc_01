...
class westernunion
{
	...
	function westernunion()
	{
		...
		// $this->email_footer = defined('BANKINFO_MASTER') ? BANKINFO_MASTER : MODULE_PAYMENT_WESTERNUNION_TEXT_EMAIL_FOOTER;
		//TODO: check send or not
		//Update: 2014/5/8 split no keyword bankinfo
		//Update: 2014/6/26 add check first entry page is products_info and click jcoupon
// 		$allow_pages = explode(',', JFOLLOWING_ALLOW_FIRST_PAGE);
// 		if ($cookie->send_email == 1)
// 		{
// 			if (!in_array($cookie->first_page, $allow_pages)/* ||
// 				$_SESSION['J_COUPON_VALIDATE'] !== true*/)
// 			{
// 				update_customer_follow_active($cookie->customer_id, false);
// 			}
// 		}
...
// 				$this->email_footer = constant('BANKINFO_NO_KEYWORD');
			
// 			if ($_SESSION['J_COUPON_VALIDATE'] === true)
// 			{
// 				if (empty($cookie->search_keyword))
// 					$this->email_footer = constant('BANKINFO_YES_COUPON_NO_KEY');
// 				else
// 					$this->email_footer = constant('BANKINFO_YES_COUPON_YES_KEY');
// 			}
// 			else
// 			{
// 				if (empty($cookie->search_keyword))
// 					$this->email_footer = constant('BANKINFO_NO_COUPON_NO_KEY');
// 				else
// 					$this->email_footer = constant('BANKINFO_NO_COUPON_YES_KEY');
// 			}
// 		}
// 		else
// 		{
// 			$this->email_footer = BANKINFO_OTHER;
// 		}
		
		
// 		if (empty($this->email_footer))
// 		{
// 			if (defined('BANKINFO_' . $cookie->account_level))
// 			{
// 				$this->email_footer = constant('BANKINFO_' . $cookie->account_level);
// 			}
// 			else
// 			{
// 				$this->email_footer = defined('BANKINFO_MASTER') ? BANKINFO_MASTER : MODULE_PAYMENT_WESTERNUNION_TEXT_EMAIL_FOOTER;
// 			}
// 		}
		$this->email_footer = get_bankinfo_payment();
	}
	// class methods
	...
	{
		return array(
			'title' => MODULE_PAYMENT_WESTERNUNION_TEXT_DESCRIPTION 
		);
	}
	...
	{
		return array(
			'MODULE_PAYMENT_WESTERNUNION_STATUS',
			'MODULE_PAYMENT_WESTERNUNION_RECEIVER_FIRST_NAME',
			'MODULE_PAYMENT_WESTERNUNION_RECEIVER_LAST_NAME',
			'MODULE_PAYMENT_WESTERNUNION_RECEIVER_ADDRESS',
			'MODULE_PAYMENT_WESTERNUNION_RECEIVER_ZIP',
			'MODULE_PAYMENT_WESTERNUNION_RECEIVER_CITY',
			'MODULE_PAYMENT_WESTERNUNION_RECEIVER_COUNTRY',
			'MODULE_PAYMENT_WESTERNUNION_RECEIVER_PHONE',
			'MODULE_PAYMENT_WESTERNUNION_SORT_ORDER',
			'MODULE_PAYMENT_WESTERNUNION_ORDER_STATUS_ID' 
		);
	}
}
