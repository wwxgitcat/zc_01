...
class ot_gv
{
	...
	/**
	 * check system to see if GVs should be made available or not.
	 * If true, then supply GV-selection fields on checkout pages
	 */
	...
	function calculate_deductions($order_total)
	{
		...
		switch($this->calculate_tax)
		{
			case 'None':
				...
				$tax_deduct = 0;
				/*
				 * if ($this->include_tax) { reset($order->info['tax_groups']); foreach ($order->info['tax_groups'] as $key=>$value) { $od_amount['tax_groups'][$key] = $order->info['tax_groups'][$key] * $ratio_tax; $tax_deduct += $od_amount['tax_groups'][$key]; } }
				 */
